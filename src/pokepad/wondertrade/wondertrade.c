...
u16 wondertrade_pokemon_gold[] = {
    ...
    POKEMON_PANZAERON, POKEMON_LOCKSCHAL, POKEMON_WELUNO, POKEMON_SEEKID,
    POKEMON_KECLEON, 0xFFFF
};

u16 wondertrade_pokemon_platinum[] = {
    POKEMON_CHANEIRA, POKEMON_IGNIVOR, POKEMON_PORYGON, POKEMON_LAPRAS,
...
void wondertrade_init_components(){
    ...
    if(fmem->wtrade_mem->usable){
        if (fmem->wtrade_mem->error_m){
            ...
            tbox_print_string(POKEPAD_WONDERTRADE_TBOX_TRADE, 2, 0, 0, 0, 0, wondertrade_fontcolmap, 0x0, str_wondertrade_do_ref);
