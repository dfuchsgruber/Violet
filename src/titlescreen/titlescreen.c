...
void ts_setup_pal(){
    pal_load_uncomp(gfx_ts_titlePal, 0, 12*32);
    pal_load_uncomp(gfx_ts_bgPal, 12*16, 32);
    pal_load_uncomp(gfx_ts_deoxys_0Pal, 13*16, 32);
    pal_load_uncomp(gfx_ts_start_higherPal, 14*16, 32);
    pal_load_uncomp(gfx_ts_start_lowerPal, 15*16, 32);
}
...
void ts_setup_bg(){
    ...
    //Load the tilesets
    bg_decomp_and_load(0, gfx_ts_titleTiles, 0, 0, false);
    bg_decomp_and_load(1, gfx_ts_deoxys_0Tiles, 0, 0, false);
    bg_decomp_and_load(2, gfx_ts_start_higherTiles, 0, 0, false);
    bg_decomp_and_load(2, gfx_ts_start_lowerTiles, 0, 13, false);
    bg_decomp_and_load(3, gfx_ts_bgTiles, 0, 0, false);
    ...
    bg_decomp_and_load(0, gfx_ts_titleMap, 0, 0, true);
    bg_decomp_and_load(1, gfx_ts_deoxys_0Map, 0, 0, true);
    bg_decomp_and_load(2, gfx_ts_start_higherMap, 0, 0, true);
    bg_decomp_and_load(2, gfx_ts_start_lowerMap, 0, 36, true);
    bg_decomp_and_load(3, gfx_ts_bgMap, 0, 0, true);
...
void ts_color_anim(u16 *vars){
    if(++(vars[3]) > 7){//4 as frames between color change
        ...
        int i;
        for (i = 6; i < 12; i++){
            color n = alpha_blend(gfx_ts_bgPal[i], col_pure_violet, (u8)vars[5]);
...
void ts_animation_sprite(u16 *vars){
    ...
    if(++(vars[6]) > 7){
        ...
        //Vscroll
        set_io(0x16, ts_deoxysScroll[vars[7]]);
...
void ts_animation_step(u16 *vars){
    ...
    u8 ts_main_cb_id = get_callback_id_by_func(ts_main_big_cb);
