...
u16 pokemon_get_evolution(pokemon *p, u8 type, u16 arg){
    u16 species = (u16)get_pokemons_attribute(p, ATTRIBUTE_SPECIES, 0);
    u16 held_item = (u16)get_pokemons_attribute(p, ATTRIBUTE_ITEM, 0);
    dprintf("Pokemon get evolution called with type %d\n", type);
    switch(type){
        case TYPE_LEVEL_UP:{ //The usual case
            arg = (u8)get_pokemons_attribute(p, ATTRIBUTE_LEVEL, 0);
            if(species == POKEMON_EVOLI){
                ...
                u8 friendship = (u8)get_pokemons_attribute(p, ATTRIBUTE_HAPPINESS, 0);
                if(friendship >= 220){
                    int j;
                    for(j = 0; j < 4; j++){
                        u16 attack = (u16)get_pokemons_attribute(p, (u8)(ATTRIBUTE_ATTACK1 + j), 0);
            ...
            for(i = 0; i < 5; i++){
                switch(pokemon_evolutions[species][i].method){
                    ...
                    case EVOLUTION_METHOD_ATK_GT_DEF:{
                        u16 atk = (u16)get_pokemons_attribute(p, ATTRIBUTE_ATK, 0);
                        u16 def = (u16)get_pokemons_attribute(p, ATTRIBUTE_DEF, 0);
                    ...
                    case EVOLUTION_METHOD_PID_EVEN:{
                        u32 pid = get_pokemons_attribute(p, ATTRIBUTE_PID, 0);
                        if(pokemon_evolutions[species][i].condition <= arg &&
                                pid % 10 <= 4)
                            return pokemon_evolutions[species][i].target;
                        break;
                    ...
                    case EVOLUTION_METHOD_HOLD_ITEM:{
                        if(held_item == pokemon_evolutions[species][i].condition){
                            ...
                            set_pokemons_attribute(p, ATTRIBUTE_ITEM, &held_item);
                            return pokemon_evolutions[species][i].target;
                        }
                        break;
                    }
