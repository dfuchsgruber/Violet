...
void bsc_command_x04_critcalc() {
	...
	if (battlers[attacking_battler].status2 & STATUS2_FOCUS_ENERGY) critical_level += 2;
	...
	if (move_effect == ATTACK_EFFECT_HIGH_CRITICAL ||
			...
			move_effect == ATTACK_EFFECT_POISON_TAIL) critical_level++;
	if (hold_effect == HOLD_EFFECT_SCOPE_LENS) critical_level++;
	if (hold_effect == HOLD_EFFECT_LUCKY_PUNCH &&
			battlers[attacking_battler].species == POKEMON_CHANEIRA) critical_level += 2;
	...
	if (battlers[defending_battler].ability != KAMPFPANZER &&
			...
			!(rnd16() % critical_hit_chances[critical_level])) critical_hit_multiplier = 2;
	else critical_hit_multiplier = 1;
