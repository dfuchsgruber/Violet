...
2. Add or edit a JSONC `.const` file under `Violet/constants/`.
...
7. Validate by generating the headers with make; generated headers are ignored by git in this repo.
...
- Constants files are JSONC, not C headers. Do not add hand-maintained `#define` constants when a `.const` table is appropriate.
