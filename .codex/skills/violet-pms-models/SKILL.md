...
7. Validate by compiling the model, checking JSON, and building the generated assembly/object through `PYMAP2S` or the existing make target.
...
- PMS JSON should store named constants as strings when the model field has an associated constant table.

## Fast Checks

```bash
python -m py_compile Violet/models/<model>.py
python -m json.tool Violet/src/path/to/file.pms >/dev/null
