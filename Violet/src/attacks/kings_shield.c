...
bool protect_attack_succeeds() {
    ...
    if (BATTLE_STATE2->status_custom[defending_battler] &
        ...
        if (!(attacks[active_attack].flags & AFFECTED_BY_PROTECT))
            return true;
        dprintf("Category of attack is %d\n", attacks[active_attack].category);
        if (attacks[active_attack].category == CATEGORY_STATUS)
            return true;
        if (attack_needs_charging(active_attack) && !(attacker->status2 &
                                                      STATUS2_ATTACK_CHARGED))
            return true;
...
void bsc_push_kings_shield_drop() {
    battler *attacker = &battlers[attacking_battler];
    if ((BATTLE_STATE2->status_custom[attacking_battler] & CUSTOM_STATUS_KINGS_SHIELD_DROP) && attacker->stat_changes[1] > 0) {
        if (attacker->stat_changes[1] == 1)
            attacker->stat_changes[1] = 0;
        else
            attacker->stat_changes[1] = (u8)(attacker->stat_changes[1] - 2);
