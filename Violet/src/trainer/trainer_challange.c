...
bool special_x36_check_loaded_trainerflag() {
    switch (fmem.current_trainer) {
        case 0:
            return checkflag(trainer_get_flag());
        case 1:
            return checkflag(trainerB_get_flag());
    }
...
u8 *trainer_get_challange_message() {
    ...
    switch (fmem.current_trainer) {
        case 0:
            msg = trainer_vars.challange_text;
            break;
        case 1:
            msg = fmem.trainer_varsB.challange_text;
            break;
    }
...
bool trigger_aggressive_wild_pokemons() {
    ...
    for (u8 i = 0; i < 16; i++) {
        if ((npcs[i].flags.active & 1) && (npcs[i].trainer_trigger == 1 || npcs[i].trainer_trigger == 3)) {
            ...
            if (num == 2)
                break; // The wild battle already is a double battle
            if (num == 0)
                continue;
            if (num == 1 && player_party_get_double_battle_viability())
                break; // Don't search for other aggresive wild, player cant do a double battle
        }
    }
    fmem.current_trainer = 0;
...
bool trigger_trainerbattles() {
    ...
    for (u8 i = 0; i < 16; i++) {
        if ((npcs[i].flags.active & 1) && (npcs[i].trainer_trigger == 1 || npcs[i].trainer_trigger == 3)) {
            ...
            if (num == 2)
                break; // The trainer is a double battle, i.e. don't search for other trainers
            if (num == 0)
                continue;
            if (num == 1 && player_party_get_double_battle_viability())
                break; // Don't search for other trainers, player can't handle it
        }
    }
...
u8 *trainer_configure_by_overworld_script(u8 *ow_script) {
    if (checkflag(FLAG_PLAYER_PARTY_STOLEN))
        return ow_script_trainerbattle_pirate_challange_after_approach;
    ...
    if (kind != TRAINER_BATTLE_CONTINUE_SCRIPT_NO_MUSIC && kind != TRAINER_BATTLE_CONTINUE_SCRIPT_DOUBLE_NO_MUSIC &&
        ...
        switch (trainers[trainer_idx].trainerclass) {
            case TRAINERCLASS_TEAM_VIOLET:
                music = MUS_VIOLET_ENCOUNTER;
                break;
            case TRAINERCLASS_REVOLUTIONAER:
                music = MUS_REVOLUTION;
                break;
