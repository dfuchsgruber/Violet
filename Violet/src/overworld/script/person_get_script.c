...
extern const u8 ow_script_move_tutor[];
...
const u8 *person_get_script(u8 target_idx, u8 map_id, u8 bank) {
	...
	switch(p->script_std) {
		case PERSON_ITEM:
			...
			*var_access(0x8001) = 1;
			return p->script ? p->script : ow_script_person_pokeball;
		...
		case PERSON_EGG:
			// DEBUG("Person egg\n");
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_person_egg;
		case PERSON_POKEMON:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_person_pokemon;
		case PERSON_POKEMON_NON_FACING:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_person_pokemon_non_facing;
		case PERSON_AGGRESSIVE_POKEMON:
			*var_access(0x8000) = p->value;
			*var_access(0x8001) = p->argument;
			return p->script ? p->script : ow_script_aggressive_wild;
		case PERSON_BERRY_TREE:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_berry_tree;
		case PERSON_MUSHROOM:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_mushroom;
		case PERSON_SHELL:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_shell;
		case PERSON_TRASH_CAN:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_trash_can;
		case PERSON_PUZZLE_BOULDER:
			return p->script ? p->script : ow_script_boulder_gym_puzzle;
		case PERSON_HAY_BALE:
			return p->script ? p->script : ow_script_boulder_hay_bale;
		case PERSON_ACCESSIBLE_MOVE_TUTOR:
			*var_access(VAR_ACCESIBLE_MOVE_TUTOR_TYPE) = (u16)(p->value + 1);
			return p->script ? p->script : ow_script_person_accessible_move_tutor;
		...
		case PERSON_SECRET_POWER_VINE:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_dungeon2_enter_forest;
		case PERSON_STATIC_BERRY_TREE:
			*var_access(0x8000) = p->value;
			*var_access(0x8001) = p->argument;
			return p->script ? p->script : ow_script_static_berry_tree;
		case PERSON_SECRET_POWER_CAVE:
			*var_access(0x8000) = p->value;
			return p->script ? p->script : ow_script_dungeon2_enter_cave;
		case PERSON_RECIPE:
			*var_access(0x8000) = p->flag;
			return p->script ? p->script : ow_script_recipe;
		case PERSON_MOVE_TUTOR:
			*var_access(0x8000) = p->value;
