...
static const healing_place2_t healing_places[NUM_HEALING_PLACES] = {
    ...
    [HEALINGPLACE_KASKADA] = {.bank = 8, .map = 0, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_ORINA_CITY] = {.bank = 9, .map = 1, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_INFERIOR] = {.bank = 10, .map = 12, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_LAUBDORF] = {.bank = 12, .map = 0, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_CEOMETRIA] = {.bank = 19, .map = 0, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_HAWEILAND] = {.bank = 27, .map = 3, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_BBSHIP] = {.bank = 33, .map = 0, .x = 3, .y = 5, .target_person = 0xFF},
    [HEALINGPLACE_TANNS_LAB] = {.bank = 35, .map = 0, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
    [HEALINGPLACE_DESERT] = {.bank = 37, .map = 0, .x = HEALINGPLACE_POKESTOP_X, .y = HEALINGPLACE_POKESTOP_Y, .target_person = 1},
};
...
void whiteout_callback_print_text(u8 self) {
    ...
    case 1: {
        ...
        case HEALINGPLACE_AMONIA:
            str = str_whiteout_mother;
            break;
        case HEALINGPLACE_BBSHIP:
            str = str_whiteout_bbship;
            direction = DIR_DOWN;
            break;
        default:
            str = str_whiteout;
    ...
    case 3: {
        if (overworld_fading_effect_finished()) {
            ...
            switch (csave.healing_place_idx) {
            case HEALINGPLACE_AMONIA:
                script = ow_script_whiteout_mother;
                break;
            case HEALINGPLACE_BBSHIP:
                script = ow_script_whiteout_bbship;
                break;
            default:
                script = ow_script_whiteout_joy;
                break;
            }
