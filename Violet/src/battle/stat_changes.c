    ...
    if (WEATHER_HAS_EFFECT) {
        switch (battlers[attacking_battler].ability) {
            ...
            case REGENMUT:
                ...
                break;
            case KAELTEWAHN:
                if (battle_weather & BATTLE_WEATHER_HAIL_ANY)
                    satk += satk / 2;
                break;
            case UNGEBROCHEN:
                if (battlers[attacking_battler].current_hp <=
                    battlers[attacking_battler].max_hp / 2)
                    satk += satk / 3;
                break;
            case HITZEWAHN:
                if (battlers[attacking_battler].status1 & STATUS1_BURNED)
                    satk += satk / 2;
                break;
        }
    }
...
int attacker_modify_atk(int atk) {
    if (WEATHER_HAS_EFFECT) {
        switch (battlers[attacking_battler].ability) {
            case SANDHERZ:
                if (battle_weather & BATTLE_WEATHER_SANDSTORM_ANY)
                    atk += atk / 2;
                break;
            case UNGEBROCHEN:
                ...
                break;
            case GIFTWAHN:
                if (battlers[attacking_battler].status1 & STATUS1_POISONED_ANY)
                    atk += atk / 2;
                break;
        }
    }
