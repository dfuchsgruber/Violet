    ...
    if (battle_flags & BATTLE_TRAINER) {
        if (battle_flags & BATTLE_LINK) {
            if (battle_flags & BATTLE_MULTI)
                return str_multi_link_intro;
            else
                return str_link_intro;
        } else {
            if (battle_flags & BATTLE_TWO_TRAINERS)
                return str_single_double_trainer_intro;
            else
                return str_single_trainer_intro;
        }
    } else {
...
u8 *battle_send_out_buffer_message() {
    if (battler_is_opponent(active_battler)) {
        if (battle_flags & BATTLE_DOUBLE) {
            if (battle_flags & BATTLE_MULTI)
                return str_multi_sends_out;
            else if (battle_flags & BATTLE_TWO_TRAINERS)
                return str_double_trainer_sends_out;
            else if (battle_flags & BATTLE_LINK)
                return str_link_opponent_sends_out_double;
            else
                return str_single_trainer_sends_out_two;
        } else {
            if (battle_flags & BATTLE_LINK)
                return str_link_opponent_send_out_single;
            else
                return str_trainer_sends_out_single;
        }
    } else {
        if (battle_flags & BATTLE_DOUBLE) {
            if (battle_flags & BATTLE_MULTI)
                return str_player_and_link_partner_send_out;
            else if (battle_flags & BATTLE_ALLY)
                return str_player_and_partner_send_out;
            else
                return str_player_send_out_double;
        } else
            return str_player_send_out_single;
    }
}
...
u8 *battle_string_return_pokemon_buffer_message() {
    if (battler_is_opponent(active_battler)) {
        if (trainer_vars.trainer_id == 0x800) {
            if (battle_flags & BATTLE_MULTI)
                return str_return_multi_trainer;
            else
                return str_return_link_trainer;
            ...
            return str_return_second_trainer;
        else
            return str_return_trainer;
    } else {
        if (battle_flags & BATTLE_ALLY)
            return str_return_ally;
        else if (battle_state->hp_scale == 0)
            return str_return_hp_scale_0;
        else if (battle_state->hp_scale == 1 || (battle_flags & BATTLE_DOUBLE))
            return str_return_hp_scale_1;
        else if (battle_state->hp_scale == 2)
            return str_return_hp_scale_2;
...
u8 *battle_switch_in_buffer_message() {
    if (battler_is_opponent(battle_scripting.battler_idx)) {
        if (battle_flags & BATTLE_LINK) {
            if (battle_flags & BATTLE_MULTI)
                return str_multi_opponent_switch_in;
            else
                return str_link_opponent_switch_in;
        } else if ((battle_flags & BATTLE_DOUBLE) && (battle_flags & BATTLE_TWO_TRAINERS) &&
                   battle_scripting.battler_idx == BATTLE_POSITION_OPPONENT_RIGHT)
            return str_second_opponent_switch_in;
        else
            return str_opponent_single_switch_in;
    } else {
        ...
        if ((battle_flags & BATTLE_DOUBLE) || battle_state->hp_scale == 0)
            return str_player_switch_in_hp_scale_0;
        else if (battle_state->hp_scale == 1)
            return str_player_switch_in_hp_scale_1;
        else if (battle_state->hp_scale == 2)
            return str_player_switch_in_hp_scale_2;
        else
            return str_player_switch_in_hp_scale_x;
    }
}
...
u8 *battle_end_battle_buffer_message() {
    if (bsc_string_buffer0[0] & BATTLE_RESULT_LINK_BATTLE_OPPONENT_RAN) {
        ...
        if (bsc_string_buffer0[0] == BATTLE_RESULT_LOST || bsc_string_buffer0[0] == BATTLE_RESULT_DRAW)
            return str_escaped;
        else if (battle_flags & BATTLE_MULTI)
            return str_multi_flee;
        else if (trainer_vars.trainer_id == 0xC00)
            return str_trainer_flee; // ??
        else
            return str_link_flee;

    } else {
        if (battler_is_opponent(active_battler) && bsc_string_buffer0[0] != BATTLE_RESULT_DRAW)
            bsc_string_buffer0[0] ^= BATTLE_RESULT_WON | BATTLE_RESULT_LOST; // The player's outcome is complementary to the foe's (of course...)
        if (battle_flags & BATTLE_MULTI) {
            if (bsc_string_buffer0[0] == BATTLE_RESULT_LOST)
                return str_multi_lost;
            else if (bsc_string_buffer0[0] == BATTLE_RESULT_WON)
                return str_multi_won;
            else
                return str_multi_drew;
        } else {
            if (trainer_vars.trainer_id == 0xC00) { // ??
                if (bsc_string_buffer0[0] == BATTLE_RESULT_WON)
                    return str_single_trainer_won;
                else if (bsc_string_buffer0[0] == BATTLE_RESULT_LOST)
                    return str_single_trainer_lost;
                else
                    return str_single_trainer_draw;
            } else {
                if (bsc_string_buffer0[0] == BATTLE_RESULT_WON)
                    return str_link_trainer_won;
                else if (bsc_string_buffer0[0] == BATTLE_RESULT_LOST)
                    return str_link_trainer_lost;
                else
                    return str_link_trainer_draw;
            }
        }
    }
}
    ...
    if (battle_flags & BATTLE_TRAINER) {
        switch (owner) {
            case OWNER_TRAINER_A:
                return trainer_vars.trainer_id;
            case OWNER_TRAINER_B:
                return fmem.trainer_varsB.trainer_id;
            case OWNER_ALLY:
                return fmem.ally_trainer_idx;
        }
    }
    return 0;
}
