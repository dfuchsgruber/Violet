...
int map_wildbattle_init(bdata current, u16 behaviour_previous_tile) {
    ...
    if (!habitat)
        return 0;
    DEBUG("Wild pokemon habitat %p, pdf_type %d\n", habitat, pdf_type);
    ...
    if (behaviour != behaviour_previous_tile && (rnd16() % 100) >= 60)
        return 0;
    DEBUG("Behaviour %d, previous tile behaviour %d\n", behaviour, behaviour_previous_tile);
    if (!wildbattle_does_pokemon_appear_by_frequency(habitat->frequency, 0)) {
        wildbattle_increase_chance(habitat->frequency);
        return 0;
    }
    DEBUG("Wild pokemon appears by frequency %d\n", habitat->frequency);
    ...
    DEBUG("Wild pokemon initialized by habitat %p, pdf_type %d\n", habitat, pdf_type);
