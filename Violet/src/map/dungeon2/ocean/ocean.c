...
map_event_header_t *dungeon2_init_events_ocean(dungeon_generator2 *dg2) {
    ...
    cpuset(&zero, &(fmem.dpersons[0]), CPUSET_FILL | CPUSET_HALFWORD | CPUSET_HALFWORD_SIZE(sizeof(map_event_person)));
    ...
    for (int i = 0; i < num_items; i++) {
        int person_idx = i + 1;
        int node_idx = i + 2;
        cpuset(&zero, &(fmem.dpersons[person_idx]), CPUSET_FILL | CPUSET_HALFWORD | CPUSET_HALFWORD_SIZE(sizeof(map_event_person)));
