...
void dungeon2_print_map_with_nodes(u8 *map, dungeon_generator2 *dg2, int nodes[][2], size_t num_nodes) {
    ...
    for (int y = 0; y < dg2->height; y++) {
        ...
        for (x = 0; x < dg2->width; x++) {
            if (map[y * dg2->width + x] == DG2_SPACE)
                charmap[x] = ' ';
...
u8 *dungeon2_create_connected_layout(dungeon_generator2 *dg2, bool random_nodes) {
    ...
    cpuset(&_dg2_wall, map1, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
    cpuset(&_dg2_wall, map2, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
    cpuset(&_dg2_wall, map_paths, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
