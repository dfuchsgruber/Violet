...
static void dungeon2_create_connected_layout_callback(u8 self) {
    ...
    switch (state->state) {
        case 0: { // Initialize maps
            ...
            cpuset(&_dg2_wall, state->map1, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
            cpuset(&_dg2_wall, state->map2, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
            cpuset(&_dg2_wall, state->map_paths, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
            state->state++;
            break;
        }
        case 1: {
...
static void dungeon2_create_patch_layout_callback(u8 self) {
    ...
    switch (vars[0]) {
        case 0: {
            ...
            cpuset(&_dg2_space, dst, CPUSET_HALFWORD | CPUSET_FILL | CPUSET_HALFWORD_SIZE(dg2->width * dg2->height));
            vars[0]++;
            vars[1] = 0;
            break;
        }
