...
bool rtc_test() {
    ...
    rtc_timestamp t;
    time_read(&t);
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.year))
        return false;
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.month))
        return false;
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.day))
        return false;
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.hour))
        return false;
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.minute))
        return false;
    if (!SANITY_IS_TWO_DIGIT_DECIMAL(t.second))
        return false;
