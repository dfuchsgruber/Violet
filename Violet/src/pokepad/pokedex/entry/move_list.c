...
static const oam_template templates_icon_egg[] = {
    {
        .tiles_tag = 0xFFFF,
        .pal_tag = POKEDEX_ENTRY_PAGE_MOVE_LIST_EXTRA_GFX_TAG_ICON,
        .oam = &sprite_egg,
        .animation = animation_egg_frames,
        .graphics = graphics_egg,
        .rotscale = oam_rotscale_anim_table_null,
        .callback = oam_null_callback,
    },
    {
        .tiles_tag = 0xFFFF,
        .pal_tag = POKEDEX_ENTRY_PAGE_MOVE_LIST_EXTRA_GFX_TAG_ICON + 1,
        .oam = &sprite_egg,
        .animation = animation_egg_frames,
        .graphics = graphics_egg,
        .rotscale = oam_rotscale_anim_table_null,
...
static void pokedex_entry_page_move_list_extra_cursor_move_callback(int idx, u8 on_initialize, UNUSED list_menu *list) {
    ...
    case POKEDEX_ENTRY_PAGE_MOVE_LIST_EXTRA_EGG: {
        ...
        oam_idx_new_icon = oam_new_forward_search(templates_icon_egg +
                                                      pokedex_entry_state->move_list_extra_oam_setup_flip_flop,
                                                  206, 122, 0);
    ...
    case POKEDEX_ENTRY_PAGE_MOVE_LIST_EXTRA_TM_HM: {
        ...
        u16 item_idx = IS_TM(item.fields.argument) ? (u16)TM_IDX_TO_ITEM_IDX(item.fields.argument) : HM_IDX_TO_ITEM_IDX(item.fields.argument - NUM_TMS);
        pal_decompress(item_get_resource(item_idx, true),
    ...
    if (str) {
        tbox_print_string(pokedex_entry_state->page_tbox_idxs[POKEDEX_ENTRY_PAGE_MOVE_LIST_TBOX_METHOD],
                          0, (u16)((8 * pokedex_entry_page_move_list_tboxes[POKEDEX_ENTRY_PAGE_MOVE_LIST_TBOX_METHOD].w - string_get_width(0, str, 0)) / 2), 36, 0, 0, &font_colormap_white, 0, str);
    } else {
        tbox_sync(pokedex_entry_state->page_tbox_idxs[POKEDEX_ENTRY_PAGE_MOVE_LIST_TBOX_METHOD], TBOX_SYNC_SET);
    }
...
static void pokedex_entry_page_move_list_setup_extra_moves() {
    ...
    DEBUG("Basis stage %d, egg moves at 0x%x\n", pokemon_get_basis_stage(pokedex_entry_state->species), egg_moves);
    if (egg_moves) {
    ...
    for (u8 i = 0; i < NUM_MOVE_TUTORS; i++) {
        DEBUG("Move tutor %d: %d\n", i, (pokemon_move_tutor_compatibility[pokedex_entry_state->species] & (1 << i)) > 0);
        if (pokemon_move_tutor_compatibility[pokedex_entry_state->species] & (1 << i)) {
    ...
    if (accessible_moves) {
        for (size_t i = 0; i < num_accessible_moves; i++) {
            ...
            DEBUG("Accesible move %d: %d, at index %d / %d\n", i, accessible_moves[i], idx, num_moves);
            string_decrypt(pokedex_entry_state->move_list_strs + idx * POKEDEX_ENTRY_PAGE_MOVE_LIST_MAX_NUM_CHARS,
                           str_move_accessible);
            pokedex_entry_state->move_list_items[idx] = (list_menu_item){
                .idx = (pokedex_entry_page_move_list_extra_item_t){
