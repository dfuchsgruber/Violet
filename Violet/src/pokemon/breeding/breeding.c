...
static u16 pokemon_breeding_feature_generator() {
    u16 rnd = rnd16();
    if (checkflag(FLAG_EGGS_IMPROVED_FEATURE_GENERATOR)) {
        rnd = rnd % 192;
    } else {
        rnd = rnd % 512;
    }
...
void breeding_pokemon_new(pokemon *p, u16 species, daycare_stru *daycare) {
    (void)(daycare);
    pokemon_new_by_prng(
        p, species, 5, 32, true, csave.daycare_offspring_pid, false, 0,
        pokemon_breeding_feature_generator, NULL);
    u8 ball = BALL_POKE;
...
u16 breeding_get_cycle_steps() {
    ...
    if (checkflag(FLAG_EGGS_HATCH_FASTER)) {
        base_cycle = base_cycle * 2 / 3;
    }
...
bool box_pokemon_hatching_proceed(box_pokemon *egg, bool consider_zero_cycles,
    ...
    if (!box_pokemon_get_attribute(egg, ATTRIBUTE_IS_EGG, 0))
        return false;
    if (box_pokemon_get_attribute(egg, ATTRIBUTE_SANITY_IS_BAD_EGG, 0))
        return false;
    ...
    if ((cycles == BREEDING_CYCLES_HATCH_IMMEDIATLEY && consider_immediate_hatching) ||
        (consider_zero_cycles && cycles == 0))
        return true;
    if (cycles != BREEDING_CYCLES_HATCH_IMMEDIATLEY)
        cycles = MAX(0, cycles - proceed_cycles);
