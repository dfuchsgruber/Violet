/*
 * sprite.h
 *
 *  Created on: Mar 28, 2018
 *      Author: dominik
 */

#ifndef INCLUDE_C_OVERWORLD_SPRITE_H_
#define INCLUDE_C_OVERWORLD_SPRITE_H_

/*
 * File:   overworld_sprites.h
 * Author: Domi
 *
 * Created on 16. Juni 2016, 11:14
 */


#include "types.h"
#include "oam.h"
#include "color.h"
#include "overworld/npc.h"
#include "map/event.h"

#define OVERWORLD_SPRITE_STRENGTH_BOULDER 97
#define OVERWORLD_SPRITE_ROMAN 179
#define OVERWORLD_SPRITE_TUTOR_CRYSTAL 235
#define OVERWORLD_SPRITE_MISC 236
#define OVERWORLD_SPRITE_BERRY 237
#define OVERWORLD_SPRITE_POKEMON_32_32 238
#define OVERWORLD_SPRITE_POKEMON_64_64 239

extern const gfx_frame *const ow_anim_standard_npc[];
extern const gfx_frame *const ow_anim_ho_oh[];
extern const gfx_frame *const ow_anim_hiro[];
extern const gfx_frame *const ow_anim_hiro_sit[];
extern const gfx_frame *const ow_anim_hiro_call[];
extern const gfx_frame *const ow_anim_hiro_rod[];
extern const gfx_frame *const ow_anim_hiro_call2[];
extern const gfx_frame *const ow_anim_hiro_bike[];
extern const gfx_frame *const ow_anim_none[];

extern const subsprite_table ow_formation_16_32;
extern const subsprite_table ow_formation_128_64;
extern const subsprite_table ow_formation_32_32;
extern const subsprite_table ow_formation_64_64;
extern const subsprite_table ow_formation_16_16;

extern sprite ow_final_oam_16_32;
extern sprite ow_final_oam_128_64;
extern sprite ow_final_oam_64_64;
extern sprite ow_final_oam_32_32;
extern sprite ow_final_oam_16_16;


typedef struct overworld_sprite {
    u16 tiles_tag; //Tiles tag
    u16 pal_tag; //0x1100 + PalID
    u16 unknown; //usually 0x11FF
    u16 size; //in bytes, e.g. for 32x16 := 0x100
    u16 width;
    u16 height;
    u8 palette_slot : 4;
    u8 shadow_size : 2;
    u8 inanimate : 1;
    u8 disable_reflection : 1; // unused in violet
    u8 tracks;
    u8 field_E;
    u8 field_F;
    const sprite *final_oam; //NSE := Pointer 1
    const subsprite_table *subsprite_table; //NSE := Pointer 2
    const gfx_frame *const *gfx_animation; //NSE := Anim Pointer
    const graphic *graphics; //NSE := equals Pointer 3
    const rotscale_frame *const *rotscale_animation; //No NSE equal
} overworld_sprite;


enum {
    OW_PAL_TAG_POKEMON_BASE = 0x2000,
    OW_PAL_TAG_POKEMON_END = 0x2200,
    OW_PAL_TAG_TUTOR_CRYSTAL_BASE = 0x2900,
    OW_PAL_TAG_MUSHROOM = 0x2E00,
    OW_PAL_TAG_SHELL,
    OW_PAL_TAG_BOULDER_GYM_PUZZLE,
    OW_PAL_TAG_MEGA_STONE,
    OW_PAL_TAG_ROMAN,
    OW_PAL_TAG_BOULDER_HAY_BALE,
    OW_PAL_TAG_RAGE_SIGN,
    OW_PAL_TAG_ROSALIE,
    OW_PAL_TAG_ICARUS,
    OW_PAL_TAG_TRASH,
    OW_PAL_TAG_SECRET_POWER_VINES,
    OW_PAL_TAG_SECRET_POWER_CAVE,
    OW_PAL_TAG_SECRET_POWER_WHIRLPOOL,
    OW_PAL_TAG_APPLE,
    OW_PAL_TAG_RECIPE,
    OW_PAL_TAG_UPSTREAM,
    OW_PAL_TAG_FAUN,
    OW_PAL_TAG_SPECIAL_SHELL,
    OW_PAL_TAG_BLUE_GEAR,
    OW_PAL_TAG_GREEN_GEAR,
    OW_PAL_TAG_RED_GEAR,
    OW_PAL_TAG_YELLOW_GEAR,
    OW_PAL_TAG_TREASURE_MAP,
    OW_PAL_TAG_PAINTER,
    OW_PAL_TAG_END = 0x3000,
};


/**
 * Returns the palette of an overworld species.
 * @param species the species to get the palette of
 * @return the palette structure
 **/
const palette *overworld_palette_get_by_species(u16 species);
/**
 * Returns the overworld sprite of an overworld species.
 * @param species the species to get the overworld sprite of
 * @return the overworld sprite.
 **/
const overworld_sprite *overworld_sprite_get_by_species(u16 species);

/**
 * Returns the overworld sprite idx that is to be used for a species.
 * @param the species to find a suitable sprite for
 * @return the overworld sprite idx to use for this species, e.g. in a person
 **/
u8 overworld_get_sprite_idx_by_species(u16 species);

/**
 * Gets the overworld sprite associated with a npc
 * @param npc the npc to get the sprite of
 * @return the overworld sprite
 **/
const overworld_sprite *overworld_get_by_npc(const npc *n);

/**
 * Returns the overworld sprite of a mushroom
 * @param mushroom_idx which mushroom to get
 * @return the overworld sprite
 **/
const overworld_sprite *overworld_sprite_get_by_mushroom_idx(u16 mushroom_idx);

/**
 * Returns the overworld palette of a mushroom
 * @return the palette sprite
 **/
const palette *overworld_palette_get_by_mushroom();

/**
 * Returns the overworld sprite of a shell
 * @param shell_idx which shell to get
 * @return the overworld sprite
 **/
const overworld_sprite *overworld_sprite_get_by_shell_idx(u16 shell_idx);

/**
 * Returns the overworld palette of a shell
 * @return the palette sprite
 **/
const palette *overworld_palette_get_by_shell();

/**
 * Returns the overworld palette of a special shell
 * @return the palette sprite
 **/
const palette *overworld_palette_get_by_special_shell();

/**
 * Returns the overworld sprite of a trash
 * @param trash_idx which trash to get
 * @return the overworld sprite
 **/
const overworld_sprite *overworld_sprite_get_by_trash_idx(u16 trash_idx);

/**
 * Returns the overworld palette of a trash
 * @return the palette sprite
 **/
const palette *overworld_palette_get_by_trash();

/**
 * Gets the sprite of a boulder.
 * @param person_script_std person's script std that indicates the boulder picture
 * @param value flags of the person
 * @return the overworld sprite
 **/
const overworld_sprite *overworld_sprite_get_by_boulder_person_script_std(u8 person_script_std, u16 value);

/**
 * Returns the overworld palette of the first gym's puzzle boulders
 * @return the palette
 **/
const palette *overworld_palette_get_gym_puzzle_boulder();

/**
 * Returns the overworld palette of the hay bale boulders for Route 3's mill.
 * @return the palette
 **/
const palette *overworld_palette_get_hay_bale_boulder();

/**
 * Returns the overworld palette of the mega stone npc
 * @return the palette
 **/
const palette *overworld_palette_get_mega_stone();

/**
 * Gets the palette of a tutor stone based on the pal tag.
 * @param tag the pal tag of the ow pal
 * @return the palette
 **/
const palette *overworld_palette_tutor_crystal_get_by_tag(u16 tag);

/**
 * Gets the overworld of a tutor crystal by its element type
 * @param type the element
 * @return the tutor crystal overworld sprite
 **/
const overworld_sprite *overworld_sprite_get_by_tutor_crystal_type(u16 type);

/**
 * Frees a npc palette if currently no active npc is using this palette.
 * @param slot the palette to free
 **/
void npc_free_palette_if_unused_by_slot(u8 slot);

/**
 * Updates the palette of a npc by, if neccesary, also loading the palette
 * @param npc the npc to update
 * @param oam the oam object to update
 **/
void overworld_npc_update_palette(npc *n, oam_object *oam);

/**
 * Sets the priority of the oam (oam attribute) according to the oams height
 * @param o the oam to set the height of
 * @param height the height
 **/
void oam_set_priority_by_height(oam_object *o, u8 height);

/**
 * Sets the subspriority (relative to other oams of the same priority) of an oam according to a height
 * @param o the oam to set the subpriority of
 * @param height the height
 * @param relative_priority priority relative to oams of the same height (simply an offset to the subpriority)
 **/
void oam_set_subpriority_by_height(u8 height, oam_object *o, u8 relative_priority);

/**
 * Creates a oam that would correspond to a person, but doesn't intialize any npc for it
 * @param person the person associated (mainyl used to determine the graphic shown)
 * @param a1 t.b.d.
 * @param x the x coordinate (in blocks)
 * @param y the y coordinate (in blocks)
 * @param z the height
 * @param direction in which direction the oam should face
 * @return oam_idx the idx of the oam created
 **/
u8 overworld_create_oam_by_person(const map_event_person *person, u8 a1, s16 x, s16 y, u8 z, u8 direction);

/**
 * Creates an oam for a given overworld npc, but assigns a custom callback to it.
 * @param n the npc to base the oam on
 * @param callback the callback for the sprite
 * @param x the x coordinate
 * @param y the y coordinate
 * @param subpriority priority relative to other oams on this layer
 * @return the idx of the oam or 64 on failure
 **/
u8 overworld_create_oam_with_callback_by_npc(const npc *n, void (*callback)(oam_object*), s16 x, s16 y, u8 subpriority);

/**
 * Starts the boulder push animation and pushes the boulder
 * @param npc_idx the npc to move
 * @param direction in which direction
 **/
void boulder_animation_start(u8 npc_idx, u8 direction);

/**
 * Attempts to do a boulder push
 * @param x x coordinate of the boulder
 * @param y y coordinate of the boulder
 * @param direction in which direction to push the boulder
 * @return if the boulder push attempt was sucessful, i.e. if a boulder push animation was triggered
 **/
bool boulder_push_attempt(s16 x, s16 y, u8 direction);

enum {
    RAGE_SPRITE_RAGE,
    RAGE_SPRITE_QUESTION_MARK,
};

/**
 * Deletes a rage effect sprite of an npc and frees resources that are not held by other rage effect sprites
 * @param oam_idx the oam to free
 **/
void npc_delete_rage_sprite(u8 oam_idx);

/**
 * @brief Checks if a shell idx is a special shell with the current daily seed
 * 
 * @param shell_idx The shell to check
 * @return if the shell is special
 */
bool shell_is_special(u16 shell_idx);

/**
 * Gets the palette of an overworld by its tag
 * @param tag the tag of the palette
 * @return the palette
*/
const palette *overworld_npc_palette_get_by_tag(u16 tag);

extern const u8 gfx_ow_bisasamTiles[];
extern const color_t gfx_ow_bisasamPal[16];
extern const u8 gfx_ow_bisaknospTiles[];
extern const color_t gfx_ow_bisaknospPal[16];
extern const u8 gfx_ow_bisaflorTiles[];
extern const color_t gfx_ow_bisaflorPal[16];
extern const u8 gfx_ow_glumandaTiles[];
extern const color_t gfx_ow_glumandaPal[16];
extern const u8 gfx_ow_glutexoTiles[];
extern const color_t gfx_ow_glutexoPal[16];
extern const u8 gfx_ow_glurakTiles[];
extern const color_t gfx_ow_glurakPal[16];
extern const u8 gfx_ow_schiggyTiles[];
extern const color_t gfx_ow_schiggyPal[16];
extern const u8 gfx_ow_schillokTiles[];
extern const color_t gfx_ow_schillokPal[16];
extern const u8 gfx_ow_turtokTiles[];
extern const color_t gfx_ow_turtokPal[16];
extern const u8 gfx_ow_dartiriTiles[];
extern const color_t gfx_ow_dartiriPal[16];
extern const u8 gfx_ow_dartignisTiles[];
extern const color_t gfx_ow_dartignisPal[16];
extern const u8 gfx_ow_fiaroTiles[];
extern const color_t gfx_ow_fiaroPal[16];
extern const u8 gfx_ow_hornliuTiles[];
extern const color_t gfx_ow_hornliuPal[16];
extern const u8 gfx_ow_kokunaTiles[];
extern const color_t gfx_ow_kokunaPal[16];
extern const u8 gfx_ow_biborTiles[];
extern const color_t gfx_ow_biborPal[16];
extern const u8 gfx_ow_staraliliTiles[];
extern const color_t gfx_ow_staraliliPal[16];
extern const u8 gfx_ow_staraviaTiles[];
extern const color_t gfx_ow_staraviaPal[16];
extern const u8 gfx_ow_staraptorTiles[];
extern const color_t gfx_ow_staraptorPal[16];
extern const u8 gfx_ow_bronzelTiles[];
extern const color_t gfx_ow_bronzelPal[16];
extern const u8 gfx_ow_bronzongTiles[];
extern const color_t gfx_ow_bronzongPal[16];
extern const u8 gfx_ow_maehikelTiles[];
extern const color_t gfx_ow_maehikelPal[16];
extern const u8 gfx_ow_chevrummTiles[];
extern const color_t gfx_ow_chevrummPal[16];
extern const u8 gfx_ow_paragoniTiles[];
extern const color_t gfx_ow_paragoniPal[16];
extern const u8 gfx_ow_tromborkTiles[];
extern const color_t gfx_ow_tromborkPal[16];
extern const u8 gfx_ow_pikachuTiles[];
extern const color_t gfx_ow_pikachuPal[16];
extern const u8 gfx_ow_raichuTiles[];
extern const color_t gfx_ow_raichuPal[16];
extern const u8 gfx_ow_sandanTiles[];
extern const color_t gfx_ow_sandanPal[16];
extern const u8 gfx_ow_sandamerTiles[];
extern const color_t gfx_ow_sandamerPal[16];
extern const u8 gfx_ow_nidoranwTiles[];
extern const color_t gfx_ow_nidoranwPal[16];
extern const u8 gfx_ow_nidorinaTiles[];
extern const color_t gfx_ow_nidorinaPal[16];
extern const u8 gfx_ow_nidoqueenTiles[];
extern const color_t gfx_ow_nidoqueenPal[16];
extern const u8 gfx_ow_nidoranmTiles[];
extern const color_t gfx_ow_nidoranmPal[16];
extern const u8 gfx_ow_nidorinoTiles[];
extern const color_t gfx_ow_nidorinoPal[16];
extern const u8 gfx_ow_nidokingTiles[];
extern const color_t gfx_ow_nidokingPal[16];
extern const u8 gfx_ow_floetteTiles[];
extern const color_t gfx_ow_floettePal[16];
extern const u8 gfx_ow_florgesTiles[];
extern const color_t gfx_ow_florgesPal[16];
extern const u8 gfx_ow_vulpixTiles[];
extern const color_t gfx_ow_vulpixPal[16];
extern const u8 gfx_ow_vulnonaTiles[];
extern const color_t gfx_ow_vulnonaPal[16];
extern const u8 gfx_ow_pummeluffTiles[];
extern const color_t gfx_ow_pummeluffPal[16];
extern const u8 gfx_ow_knuddeluffTiles[];
extern const color_t gfx_ow_knuddeluffPal[16];
extern const u8 gfx_ow_zubatTiles[];
extern const color_t gfx_ow_zubatPal[16];
extern const u8 gfx_ow_golbatTiles[];
extern const color_t gfx_ow_golbatPal[16];
extern const u8 gfx_ow_myraplaTiles[];
extern const color_t gfx_ow_myraplaPal[16];
extern const u8 gfx_ow_duflorTiles[];
extern const color_t gfx_ow_duflorPal[16];
extern const u8 gfx_ow_giflorTiles[];
extern const color_t gfx_ow_giflorPal[16];
extern const u8 gfx_ow_folipurbaTiles[];
extern const color_t gfx_ow_folipurbaPal[16];
extern const u8 gfx_ow_feelinaraTiles[];
extern const color_t gfx_ow_feelinaraPal[16];
extern const u8 gfx_ow_wattzapfTiles[];
extern const color_t gfx_ow_wattzapfPal[16];
extern const u8 gfx_ow_voltulaTiles[];
extern const color_t gfx_ow_voltulaPal[16];
extern const u8 gfx_ow_digdaTiles[];
extern const color_t gfx_ow_digdaPal[16];
extern const u8 gfx_ow_digdriTiles[];
extern const color_t gfx_ow_digdriPal[16];
extern const u8 gfx_ow_kastadurTiles[];
extern const color_t gfx_ow_kastadurPal[16];
extern const u8 gfx_ow_tentantelTiles[];
extern const color_t gfx_ow_tentantelPal[16];
extern const u8 gfx_ow_entonTiles[];
extern const color_t gfx_ow_entonPal[16];
extern const u8 gfx_ow_entoronTiles[];
extern const color_t gfx_ow_entoronPal[16];
extern const u8 gfx_ow_menkiTiles[];
extern const color_t gfx_ow_menkiPal[16];
extern const u8 gfx_ow_rasaffTiles[];
extern const color_t gfx_ow_rasaffPal[16];
extern const u8 gfx_ow_fukanoTiles[];
extern const color_t gfx_ow_fukanoPal[16];
extern const u8 gfx_ow_arkaniTiles[];
extern const color_t gfx_ow_arkaniPal[16];
extern const u8 gfx_ow_gramoklesTiles[];
extern const color_t gfx_ow_gramoklesPal[16];
extern const u8 gfx_ow_duoklesTiles[];
extern const color_t gfx_ow_duoklesPal[16];
extern const u8 gfx_ow_durengardTiles[];
extern const color_t gfx_ow_durengardPal[16];
extern const u8 gfx_ow_abraTiles[];
extern const color_t gfx_ow_abraPal[16];
extern const u8 gfx_ow_kadabraTiles[];
extern const color_t gfx_ow_kadabraPal[16];
extern const u8 gfx_ow_simsalaTiles[];
extern const color_t gfx_ow_simsalaPal[16];
extern const u8 gfx_ow_macholloTiles[];
extern const color_t gfx_ow_macholloPal[16];
extern const u8 gfx_ow_maschockTiles[];
extern const color_t gfx_ow_maschockPal[16];
extern const u8 gfx_ow_machomeiTiles[];
extern const color_t gfx_ow_machomeiPal[16];
extern const u8 gfx_ow_knofensaTiles[];
extern const color_t gfx_ow_knofensaPal[16];
extern const u8 gfx_ow_ultrigariaTiles[];
extern const color_t gfx_ow_ultrigariaPal[16];
extern const u8 gfx_ow_sarzeniaTiles[];
extern const color_t gfx_ow_sarzeniaPal[16];
extern const u8 gfx_ow_tentachaTiles[];
extern const color_t gfx_ow_tentachaPal[16];
extern const u8 gfx_ow_tentoxaTiles[];
extern const color_t gfx_ow_tentoxaPal[16];
extern const u8 gfx_ow_kleinsteinTiles[];
extern const color_t gfx_ow_kleinsteinPal[16];
extern const u8 gfx_ow_georokTiles[];
extern const color_t gfx_ow_georokPal[16];
extern const u8 gfx_ow_geowazTiles[];
extern const color_t gfx_ow_geowazPal[16];
extern const u8 gfx_ow_ponitaTiles[];
extern const color_t gfx_ow_ponitaPal[16];
extern const u8 gfx_ow_gallopaTiles[];
extern const color_t gfx_ow_gallopaPal[16];
extern const u8 gfx_ow_flegmonTiles[];
extern const color_t gfx_ow_flegmonPal[16];
extern const u8 gfx_ow_lahmusTiles[];
extern const color_t gfx_ow_lahmusPal[16];
extern const u8 gfx_ow_magnetiloTiles[];
extern const color_t gfx_ow_magnetiloPal[16];
extern const u8 gfx_ow_magnetonTiles[];
extern const color_t gfx_ow_magnetonPal[16];
extern const u8 gfx_ow_porentaTiles[];
extern const color_t gfx_ow_porentaPal[16];
extern const u8 gfx_ow_doduTiles[];
extern const color_t gfx_ow_doduPal[16];
extern const u8 gfx_ow_dodriTiles[];
extern const color_t gfx_ow_dodriPal[16];
extern const u8 gfx_ow_jurobTiles[];
extern const color_t gfx_ow_jurobPal[16];
extern const u8 gfx_ow_jugongTiles[];
extern const color_t gfx_ow_jugongPal[16];
extern const u8 gfx_ow_sleimaTiles[];
extern const color_t gfx_ow_sleimaPal[16];
extern const u8 gfx_ow_sleimokTiles[];
extern const color_t gfx_ow_sleimokPal[16];
extern const u8 gfx_ow_muschasTiles[];
extern const color_t gfx_ow_muschasPal[16];
extern const u8 gfx_ow_austosTiles[];
extern const color_t gfx_ow_austosPal[16];
extern const u8 gfx_ow_nebulakTiles[];
extern const color_t gfx_ow_nebulakPal[16];
extern const u8 gfx_ow_alpolloTiles[];
extern const color_t gfx_ow_alpolloPal[16];
extern const u8 gfx_ow_gengarTiles[];
extern const color_t gfx_ow_gengarPal[16];
extern const u8 gfx_ow_onixTiles[];
extern const color_t gfx_ow_onixPal[16];
extern const u8 gfx_ow_traumatoTiles[];
extern const color_t gfx_ow_traumatoPal[16];
extern const u8 gfx_ow_hypnoTiles[];
extern const color_t gfx_ow_hypnoPal[16];
extern const u8 gfx_ow_krabbyTiles[];
extern const color_t gfx_ow_krabbyPal[16];
extern const u8 gfx_ow_kinglerTiles[];
extern const color_t gfx_ow_kinglerPal[16];
extern const u8 gfx_ow_voltobalTiles[];
extern const color_t gfx_ow_voltobalPal[16];
extern const u8 gfx_ow_lektrobalTiles[];
extern const color_t gfx_ow_lektrobalPal[16];
extern const u8 gfx_ow_oweiTiles[];
extern const color_t gfx_ow_oweiPal[16];
extern const u8 gfx_ow_kokoweiTiles[];
extern const color_t gfx_ow_kokoweiPal[16];
extern const u8 gfx_ow_tragossoTiles[];
extern const color_t gfx_ow_tragossoPal[16];
extern const u8 gfx_ow_knoggaTiles[];
extern const color_t gfx_ow_knoggaPal[16];
extern const u8 gfx_ow_kickleeTiles[];
extern const color_t gfx_ow_kickleePal[16];
extern const u8 gfx_ow_nockchanTiles[];
extern const color_t gfx_ow_nockchanPal[16];
extern const u8 gfx_ow_schlurpTiles[];
extern const color_t gfx_ow_schlurpPal[16];
extern const u8 gfx_ow_smogonTiles[];
extern const color_t gfx_ow_smogonPal[16];
extern const u8 gfx_ow_smogmogTiles[];
extern const color_t gfx_ow_smogmogPal[16];
extern const u8 gfx_ow_rihornTiles[];
extern const color_t gfx_ow_rihornPal[16];
extern const u8 gfx_ow_rizerosTiles[];
extern const color_t gfx_ow_rizerosPal[16];
extern const u8 gfx_ow_chaneiraTiles[];
extern const color_t gfx_ow_chaneiraPal[16];
extern const u8 gfx_ow_zobirisTiles[];
extern const color_t gfx_ow_zobirisPal[16];
extern const u8 gfx_ow_kangamaTiles[];
extern const color_t gfx_ow_kangamaPal[16];
extern const u8 gfx_ow_seeperTiles[];
extern const color_t gfx_ow_seeperPal[16];
extern const u8 gfx_ow_seemonTiles[];
extern const color_t gfx_ow_seemonPal[16];
extern const u8 gfx_ow_quabbelTiles[];
extern const color_t gfx_ow_quabbelPal[16];
extern const u8 gfx_ow_apoquallypTiles[];
extern const color_t gfx_ow_apoquallypPal[16];
extern const u8 gfx_ow_sternduTiles[];
extern const color_t gfx_ow_sternduPal[16];
extern const u8 gfx_ow_starmieTiles[];
extern const color_t gfx_ow_starmiePal[16];
extern const u8 gfx_ow_traunfugilTiles[];
extern const color_t gfx_ow_traunfugilPal[16];
extern const u8 gfx_ow_sichlorTiles[];
extern const color_t gfx_ow_sichlorPal[16];
extern const u8 gfx_ow_galagladiTiles[];
extern const color_t gfx_ow_galagladiPal[16];
extern const u8 gfx_ow_elektekTiles[];
extern const color_t gfx_ow_elektekPal[16];
extern const u8 gfx_ow_magmarTiles[];
extern const color_t gfx_ow_magmarPal[16];
extern const u8 gfx_ow_pinsirTiles[];
extern const color_t gfx_ow_pinsirPal[16];
extern const u8 gfx_ow_taurosTiles[];
extern const color_t gfx_ow_taurosPal[16];
extern const u8 gfx_ow_karpadorTiles[];
extern const color_t gfx_ow_karpadorPal[16];
extern const u8 gfx_ow_garadosTiles[];
extern const color_t gfx_ow_garadosPal[16];
extern const u8 gfx_ow_laprasTiles[];
extern const color_t gfx_ow_laprasPal[16];
extern const u8 gfx_ow_dittoTiles[];
extern const color_t gfx_ow_dittoPal[16];
extern const u8 gfx_ow_evoliTiles[];
extern const color_t gfx_ow_evoliPal[16];
extern const u8 gfx_ow_aquanaTiles[];
extern const color_t gfx_ow_aquanaPal[16];
extern const u8 gfx_ow_blitzaTiles[];
extern const color_t gfx_ow_blitzaPal[16];
extern const u8 gfx_ow_flamaraTiles[];
extern const color_t gfx_ow_flamaraPal[16];
extern const u8 gfx_ow_porygonTiles[];
extern const color_t gfx_ow_porygonPal[16];
extern const u8 gfx_ow_amonitasTiles[];
extern const color_t gfx_ow_amonitasPal[16];
extern const u8 gfx_ow_amorosoTiles[];
extern const color_t gfx_ow_amorosoPal[16];
extern const u8 gfx_ow_kabutoTiles[];
extern const color_t gfx_ow_kabutoPal[16];
extern const u8 gfx_ow_kabutopsTiles[];
extern const color_t gfx_ow_kabutopsPal[16];
extern const u8 gfx_ow_aerodactylTiles[];
extern const color_t gfx_ow_aerodactylPal[16];
extern const u8 gfx_ow_relaxoTiles[];
extern const color_t gfx_ow_relaxoPal[16];
extern const u8 gfx_ow_arktosTiles[];
extern const color_t gfx_ow_arktosPal[16];
extern const u8 gfx_ow_zapdosTiles[];
extern const color_t gfx_ow_zapdosPal[16];
extern const u8 gfx_ow_lavadosTiles[];
extern const color_t gfx_ow_lavadosPal[16];
extern const u8 gfx_ow_dratiniTiles[];
extern const color_t gfx_ow_dratiniPal[16];
extern const u8 gfx_ow_dragonirTiles[];
extern const color_t gfx_ow_dragonirPal[16];
extern const u8 gfx_ow_dragoranTiles[];
extern const color_t gfx_ow_dragoranPal[16];
extern const u8 gfx_ow_mewtuTiles[];
extern const color_t gfx_ow_mewtuPal[16];
extern const u8 gfx_ow_mewTiles[];
extern const color_t gfx_ow_mewPal[16];
extern const u8 gfx_ow_endivieTiles[];
extern const color_t gfx_ow_endiviePal[16];
extern const u8 gfx_ow_lorblattTiles[];
extern const color_t gfx_ow_lorblattPal[16];
extern const u8 gfx_ow_meganieTiles[];
extern const color_t gfx_ow_meganiePal[16];
extern const u8 gfx_ow_feurigelTiles[];
extern const color_t gfx_ow_feurigelPal[16];
extern const u8 gfx_ow_igelavarTiles[];
extern const color_t gfx_ow_igelavarPal[16];
extern const u8 gfx_ow_tornuptoTiles[];
extern const color_t gfx_ow_tornuptoPal[16];
extern const u8 gfx_ow_karnimaniTiles[];
extern const color_t gfx_ow_karnimaniPal[16];
extern const u8 gfx_ow_tyracrocTiles[];
extern const color_t gfx_ow_tyracrocPal[16];
extern const u8 gfx_ow_impergatorTiles[];
extern const color_t gfx_ow_impergatorPal[16];
extern const u8 gfx_ow_ignivorTiles[];
extern const color_t gfx_ow_ignivorPal[16];
extern const u8 gfx_ow_ramothTiles[];
extern const color_t gfx_ow_ramothPal[16];
extern const u8 gfx_ow_glaziolaTiles[];
extern const color_t gfx_ow_glaziolaPal[16];
extern const u8 gfx_ow_resladeroTiles[];
extern const color_t gfx_ow_resladeroPal[16];
extern const u8 gfx_ow_ledybaTiles[];
extern const color_t gfx_ow_ledybaPal[16];
extern const u8 gfx_ow_ledianTiles[];
extern const color_t gfx_ow_ledianPal[16];
extern const u8 gfx_ow_webarakTiles[];
extern const color_t gfx_ow_webarakPal[16];
extern const u8 gfx_ow_ariadosTiles[];
extern const color_t gfx_ow_ariadosPal[16];
extern const u8 gfx_ow_iksbatTiles[];
extern const color_t gfx_ow_iksbatPal[16];
extern const u8 gfx_ow_lampiTiles[];
extern const color_t gfx_ow_lampiPal[16];
extern const u8 gfx_ow_lanturnTiles[];
extern const color_t gfx_ow_lanturnPal[16];
extern const u8 gfx_ow_magnezoneTiles[];
extern const color_t gfx_ow_magnezonePal[16];
extern const u8 gfx_ow_togekissTiles[];
extern const color_t gfx_ow_togekissPal[16];
extern const u8 gfx_ow_flabebeTiles[];
extern const color_t gfx_ow_flabebePal[16];
extern const u8 gfx_ow_togepiTiles[];
extern const color_t gfx_ow_togepiPal[16];
extern const u8 gfx_ow_togeticTiles[];
extern const color_t gfx_ow_togeticPal[16];
extern const u8 gfx_ow_natuTiles[];
extern const color_t gfx_ow_natuPal[16];
extern const u8 gfx_ow_xatuTiles[];
extern const color_t gfx_ow_xatuPal[16];
extern const u8 gfx_ow_voltilammTiles[];
extern const color_t gfx_ow_voltilammPal[16];
extern const u8 gfx_ow_waatyTiles[];
extern const color_t gfx_ow_waatyPal[16];
extern const u8 gfx_ow_ampharosTiles[];
extern const color_t gfx_ow_ampharosPal[16];
extern const u8 gfx_ow_blubellaTiles[];
extern const color_t gfx_ow_blubellaPal[16];
extern const u8 gfx_ow_marillTiles[];
extern const color_t gfx_ow_marillPal[16];
extern const u8 gfx_ow_azumarillTiles[];
extern const color_t gfx_ow_azumarillPal[16];
extern const u8 gfx_ow_mogelbaumTiles[];
extern const color_t gfx_ow_mogelbaumPal[16];
extern const u8 gfx_ow_clavionTiles[];
extern const color_t gfx_ow_clavionPal[16];
extern const u8 gfx_ow_lichtelTiles[];
extern const color_t gfx_ow_lichtelPal[16];
extern const u8 gfx_ow_laternectoTiles[];
extern const color_t gfx_ow_laternectoPal[16];
extern const u8 gfx_ow_skelabraTiles[];
extern const color_t gfx_ow_skelabraPal[16];
extern const u8 gfx_ow_griffelTiles[];
extern const color_t gfx_ow_griffelPal[16];
extern const u8 gfx_ow_shnebedeckTiles[];
extern const color_t gfx_ow_shnebedeckPal[16];
extern const u8 gfx_ow_rexblisarTiles[];
extern const color_t gfx_ow_rexblisarPal[16];
extern const u8 gfx_ow_ambidiffelTiles[];
extern const color_t gfx_ow_ambidiffelPal[16];
extern const u8 gfx_ow_felinoTiles[];
extern const color_t gfx_ow_felinoPal[16];
extern const u8 gfx_ow_morlordTiles[];
extern const color_t gfx_ow_morlordPal[16];
extern const u8 gfx_ow_psianaTiles[];
extern const color_t gfx_ow_psianaPal[16];
extern const u8 gfx_ow_nachtaraTiles[];
extern const color_t gfx_ow_nachtaraPal[16];
extern const u8 gfx_ow_kramurxTiles[];
extern const color_t gfx_ow_kramurxPal[16];
extern const u8 gfx_ow_laschokingTiles[];
extern const color_t gfx_ow_laschokingPal[16];
extern const u8 gfx_ow_kramshefTiles[];
extern const color_t gfx_ow_kramshefPal[16];
extern const u8 gfx_ow_icognitoTiles[];
extern const color_t gfx_ow_icognitoPal[16];
extern const u8 gfx_ow_frosdedjeTiles[];
extern const color_t gfx_ow_frosdedjePal[16];
extern const u8 gfx_ow_skorgroTiles[];
extern const color_t gfx_ow_skorgroPal[16];
extern const u8 gfx_ow_tannzaTiles[];
extern const color_t gfx_ow_tannzaPal[16];
extern const u8 gfx_ow_forstellkaTiles[];
extern const color_t gfx_ow_forstellkaPal[16];
extern const u8 gfx_ow_elevoltekTiles[];
extern const color_t gfx_ow_elevoltekPal[16];
extern const u8 gfx_ow_skorglaTiles[];
extern const color_t gfx_ow_skorglaPal[16];
extern const u8 gfx_ow_stahlosTiles[];
extern const color_t gfx_ow_stahlosPal[16];
extern const u8 gfx_ow_snubbullTiles[];
extern const color_t gfx_ow_snubbullPal[16];
extern const u8 gfx_ow_granbullTiles[];
extern const color_t gfx_ow_granbullPal[16];
extern const u8 gfx_ow_mamutelTiles[];
extern const color_t gfx_ow_mamutelPal[16];
extern const u8 gfx_ow_scheroxTiles[];
extern const color_t gfx_ow_scheroxPal[16];
extern const u8 gfx_ow_fluffeluffTiles[];
extern const color_t gfx_ow_fluffeluffPal[16];
extern const u8 gfx_ow_skarabornTiles[];
extern const color_t gfx_ow_skarabornPal[16];
extern const u8 gfx_ow_kryppukTiles[];
extern const color_t gfx_ow_kryppukPal[16];
extern const u8 gfx_ow_teddiursaTiles[];
extern const color_t gfx_ow_teddiursaPal[16];
extern const u8 gfx_ow_ursaringTiles[];
extern const color_t gfx_ow_ursaringPal[16];
extern const u8 gfx_ow_schneckmagTiles[];
extern const color_t gfx_ow_schneckmagPal[16];
extern const u8 gfx_ow_magcargoTiles[];
extern const color_t gfx_ow_magcargoPal[16];
extern const u8 gfx_ow_quiekelTiles[];
extern const color_t gfx_ow_quiekelPal[16];
extern const u8 gfx_ow_keifelTiles[];
extern const color_t gfx_ow_keifelPal[16];
extern const u8 gfx_ow_mampfaxoTiles[];
extern const color_t gfx_ow_mampfaxoPal[16];
extern const u8 gfx_ow_remoraidTiles[];
extern const color_t gfx_ow_remoraidPal[16];
extern const u8 gfx_ow_octilleryTiles[];
extern const color_t gfx_ow_octilleryPal[16];
extern const u8 gfx_ow_botogelTiles[];
extern const color_t gfx_ow_botogelPal[16];
extern const u8 gfx_ow_schlurplekTiles[];
extern const color_t gfx_ow_schlurplekPal[16];
extern const u8 gfx_ow_panzaeronTiles[];
extern const color_t gfx_ow_panzaeronPal[16];
extern const u8 gfx_ow_hundusterTiles[];
extern const color_t gfx_ow_hundusterPal[16];
extern const u8 gfx_ow_hundemonTiles[];
extern const color_t gfx_ow_hundemonPal[16];
extern const u8 gfx_ow_seedrakingTiles[];
extern const color_t gfx_ow_seedrakingPal[16];
extern const u8 gfx_ow_phanpyTiles[];
extern const color_t gfx_ow_phanpyPal[16];
extern const u8 gfx_ow_donphanTiles[];
extern const color_t gfx_ow_donphanPal[16];
extern const u8 gfx_ow_porygon2Tiles[];
extern const color_t gfx_ow_porygon2Pal[16];
extern const u8 gfx_ow_voluminasTiles[];
extern const color_t gfx_ow_voluminasPal[16];
extern const u8 gfx_ow_traunmagilTiles[];
extern const color_t gfx_ow_traunmagilPal[16];
extern const u8 gfx_ow_rabauzTiles[];
extern const color_t gfx_ow_rabauzPal[16];
extern const u8 gfx_ow_kapoeraTiles[];
extern const color_t gfx_ow_kapoeraPal[16];
extern const u8 gfx_ow_elekidTiles[];
extern const color_t gfx_ow_elekidPal[16];
extern const u8 gfx_ow_magbyTiles[];
extern const color_t gfx_ow_magbyPal[16];
extern const u8 gfx_ow_miltankTiles[];
extern const color_t gfx_ow_miltankPal[16];
extern const u8 gfx_ow_heiteiraTiles[];
extern const color_t gfx_ow_heiteiraPal[16];
extern const u8 gfx_ow_raikouTiles[];
extern const color_t gfx_ow_raikouPal[16];
extern const u8 gfx_ow_enteiTiles[];
extern const color_t gfx_ow_enteiPal[16];
extern const u8 gfx_ow_suicuneTiles[];
extern const color_t gfx_ow_suicunePal[16];
extern const u8 gfx_ow_larvitarTiles[];
extern const color_t gfx_ow_larvitarPal[16];
extern const u8 gfx_ow_pupitarTiles[];
extern const color_t gfx_ow_pupitarPal[16];
extern const u8 gfx_ow_despotarTiles[];
extern const color_t gfx_ow_despotarPal[16];
extern const u8 gfx_ow_lugiaTiles[];
extern const color_t gfx_ow_lugiaPal[16];
extern const u8 gfx_ow_ho_ohTiles[];
extern const color_t gfx_ow_ho_ohPal[16];
extern const u8 gfx_ow_celebiTiles[];
extern const color_t gfx_ow_celebiPal[16];
extern const u8 gfx_ow_durengardaTiles[];
extern const color_t gfx_ow_durengardaPal[16];
extern const u8 gfx_ow_fiffyenTiles[];
extern const color_t gfx_ow_fiffyenPal[16];
extern const u8 gfx_ow_magnayenTiles[];
extern const color_t gfx_ow_magnayenPal[16];
extern const u8 gfx_ow_zigzachsTiles[];
extern const color_t gfx_ow_zigzachsPal[16];
extern const u8 gfx_ow_geradaksTiles[];
extern const color_t gfx_ow_geradaksPal[16];
extern const u8 gfx_ow_waumpelTiles[];
extern const color_t gfx_ow_waumpelPal[16];
extern const u8 gfx_ow_schalokoTiles[];
extern const color_t gfx_ow_schalokoPal[16];
extern const u8 gfx_ow_papinellaTiles[];
extern const color_t gfx_ow_papinellaPal[16];
extern const u8 gfx_ow_panekonTiles[];
extern const color_t gfx_ow_panekonPal[16];
extern const u8 gfx_ow_pudoxTiles[];
extern const color_t gfx_ow_pudoxPal[16];
extern const u8 gfx_ow_loturzelTiles[];
extern const color_t gfx_ow_loturzelPal[16];
extern const u8 gfx_ow_lombreroTiles[];
extern const color_t gfx_ow_lombreroPal[16];
extern const u8 gfx_ow_kappaloresTiles[];
extern const color_t gfx_ow_kappaloresPal[16];
extern const u8 gfx_ow_samurzelTiles[];
extern const color_t gfx_ow_samurzelPal[16];
extern const u8 gfx_ow_blanasTiles[];
extern const color_t gfx_ow_blanasPal[16];
extern const u8 gfx_ow_tengulistTiles[];
extern const color_t gfx_ow_tengulistPal[16];
extern const u8 gfx_ow_nincadaTiles[];
extern const color_t gfx_ow_nincadaPal[16];
extern const u8 gfx_ow_ninjaskTiles[];
extern const color_t gfx_ow_ninjaskPal[16];
extern const u8 gfx_ow_ninjatomTiles[];
extern const color_t gfx_ow_ninjatomPal[16];
extern const u8 gfx_ow_schwalbiniTiles[];
extern const color_t gfx_ow_schwalbiniPal[16];
extern const u8 gfx_ow_schwalbossTiles[];
extern const color_t gfx_ow_schwalbossPal[16];
extern const u8 gfx_ow_knilzTiles[];
extern const color_t gfx_ow_knilzPal[16];
extern const u8 gfx_ow_kapilzTiles[];
extern const color_t gfx_ow_kapilzPal[16];
extern const u8 gfx_ow_pandirTiles[];
extern const color_t gfx_ow_pandirPal[16];
extern const u8 gfx_ow_wingullTiles[];
extern const color_t gfx_ow_wingullPal[16];
extern const u8 gfx_ow_pelipperTiles[];
extern const color_t gfx_ow_pelipperPal[16];
extern const u8 gfx_ow_gehweiherTiles[];
extern const color_t gfx_ow_gehweiherPal[16];
extern const u8 gfx_ow_maskeregenTiles[];
extern const color_t gfx_ow_maskeregenPal[16];
extern const u8 gfx_ow_wailmerTiles[];
extern const color_t gfx_ow_wailmerPal[16];
extern const u8 gfx_ow_wailordTiles[];
extern const color_t gfx_ow_wailordPal[16];
extern const u8 gfx_ow_enecoTiles[];
extern const color_t gfx_ow_enecoPal[16];
extern const u8 gfx_ow_enekoroTiles[];
extern const color_t gfx_ow_enekoroPal[16];
extern const u8 gfx_ow_kecleonTiles[];
extern const color_t gfx_ow_kecleonPal[16];
extern const u8 gfx_ow_puppanceTiles[];
extern const color_t gfx_ow_puppancePal[16];
extern const u8 gfx_ow_lepumentasTiles[];
extern const color_t gfx_ow_lepumentasPal[16];
extern const u8 gfx_ow_nasgnetTiles[];
extern const color_t gfx_ow_nasgnetPal[16];
extern const u8 gfx_ow_magbrantTiles[];
extern const color_t gfx_ow_magbrantPal[16];
extern const u8 gfx_ow_zwirrfinstTiles[];
extern const color_t gfx_ow_zwirrfinstPal[16];
extern const u8 gfx_ow_schmerbeTiles[];
extern const color_t gfx_ow_schmerbePal[16];
extern const u8 gfx_ow_welsarTiles[];
extern const color_t gfx_ow_welsarPal[16];
extern const u8 gfx_ow_rihorniorTiles[];
extern const color_t gfx_ow_rihorniorPal[16];
extern const u8 gfx_ow_krebscorpsTiles[];
extern const color_t gfx_ow_krebscorpsPal[16];
extern const u8 gfx_ow_krebutackTiles[];
extern const color_t gfx_ow_krebutackPal[16];
extern const u8 gfx_ow_barschwaTiles[];
extern const color_t gfx_ow_barschwaPal[16];
extern const u8 gfx_ow_miloticTiles[];
extern const color_t gfx_ow_miloticPal[16];
extern const u8 gfx_ow_kanivanhaTiles[];
extern const color_t gfx_ow_kanivanhaPal[16];
extern const u8 gfx_ow_tohaidoTiles[];
extern const color_t gfx_ow_tohaidoPal[16];
extern const u8 gfx_ow_knacklionTiles[];
extern const color_t gfx_ow_knacklionPal[16];
extern const u8 gfx_ow_vibravaTiles[];
extern const color_t gfx_ow_vibravaPal[16];
extern const u8 gfx_ow_libelldraTiles[];
extern const color_t gfx_ow_libelldraPal[16];
extern const u8 gfx_ow_rioluTiles[];
extern const color_t gfx_ow_rioluPal[16];
extern const u8 gfx_ow_lucarioTiles[];
extern const color_t gfx_ow_lucarioPal[16];
extern const u8 gfx_ow_frizelblizTiles[];
extern const color_t gfx_ow_frizelblizPal[16];
extern const u8 gfx_ow_voltensoTiles[];
extern const color_t gfx_ow_voltensoPal[16];
extern const u8 gfx_ow_camaubTiles[];
extern const color_t gfx_ow_camaubPal[16];
extern const u8 gfx_ow_cameruptTiles[];
extern const color_t gfx_ow_cameruptPal[16];
extern const u8 gfx_ow_seemopsTiles[];
extern const color_t gfx_ow_seemopsPal[16];
extern const u8 gfx_ow_seejongTiles[];
extern const color_t gfx_ow_seejongPal[16];
extern const u8 gfx_ow_walraisaTiles[];
extern const color_t gfx_ow_walraisaPal[16];
extern const u8 gfx_ow_tuskaTiles[];
extern const color_t gfx_ow_tuskaPal[16];
extern const u8 gfx_ow_noktuskaTiles[];
extern const color_t gfx_ow_noktuskaPal[16];
extern const u8 gfx_ow_schneppkeTiles[];
extern const color_t gfx_ow_schneppkePal[16];
extern const u8 gfx_ow_firnontorTiles[];
extern const color_t gfx_ow_firnontorPal[16];
extern const u8 gfx_ow_lunasteinTiles[];
extern const color_t gfx_ow_lunasteinPal[16];
extern const u8 gfx_ow_sonnfelTiles[];
extern const color_t gfx_ow_sonnfelPal[16];
extern const u8 gfx_ow_azurillTiles[];
extern const color_t gfx_ow_azurillPal[16];
extern const u8 gfx_ow_spoinkTiles[];
extern const color_t gfx_ow_spoinkPal[16];
extern const u8 gfx_ow_groinkTiles[];
extern const color_t gfx_ow_groinkPal[16];
extern const u8 gfx_ow_plusleTiles[];
extern const color_t gfx_ow_pluslePal[16];
extern const u8 gfx_ow_minunTiles[];
extern const color_t gfx_ow_minunPal[16];
extern const u8 gfx_ow_flunkiferTiles[];
extern const color_t gfx_ow_flunkiferPal[16];
extern const u8 gfx_ow_meditieTiles[];
extern const color_t gfx_ow_meditiePal[16];
extern const u8 gfx_ow_meditalisTiles[];
extern const color_t gfx_ow_meditalisPal[16];
extern const u8 gfx_ow_wabluTiles[];
extern const color_t gfx_ow_wabluPal[16];
extern const u8 gfx_ow_altariaTiles[];
extern const color_t gfx_ow_altariaPal[16];
extern const u8 gfx_ow_klingplimTiles[];
extern const color_t gfx_ow_klingplimPal[16];
extern const u8 gfx_ow_zwirrlichtTiles[];
extern const color_t gfx_ow_zwirrlichtPal[16];
extern const u8 gfx_ow_zwirrklopTiles[];
extern const color_t gfx_ow_zwirrklopPal[16];
extern const u8 gfx_ow_porygon_zTiles[];
extern const color_t gfx_ow_porygon_zPal[16];
extern const u8 gfx_ow_bummelzTiles[];
extern const color_t gfx_ow_bummelzPal[16];
extern const u8 gfx_ow_muntierTiles[];
extern const color_t gfx_ow_muntierPal[16];
extern const u8 gfx_ow_letarkingTiles[];
extern const color_t gfx_ow_letarkingPal[16];
extern const u8 gfx_ow_schluppuckTiles[];
extern const color_t gfx_ow_schluppuckPal[16];
extern const u8 gfx_ow_schlukwechTiles[];
extern const color_t gfx_ow_schlukwechPal[16];
extern const u8 gfx_ow_tropiusTiles[];
extern const color_t gfx_ow_tropiusPal[16];
extern const u8 gfx_ow_kaumalatTiles[];
extern const color_t gfx_ow_kaumalatPal[16];
extern const u8 gfx_ow_knarkselTiles[];
extern const color_t gfx_ow_knarkselPal[16];
extern const u8 gfx_ow_knakrackTiles[];
extern const color_t gfx_ow_knakrackPal[16];
extern const u8 gfx_ow_perluTiles[];
extern const color_t gfx_ow_perluPal[16];
extern const u8 gfx_ow_aalabyssTiles[];
extern const color_t gfx_ow_aalabyssPal[16];
extern const u8 gfx_ow_saganabyssTiles[];
extern const color_t gfx_ow_saganabyssPal[16];
extern const u8 gfx_ow_absolTiles[];
extern const color_t gfx_ow_absolPal[16];
extern const u8 gfx_ow_shuppetTiles[];
extern const color_t gfx_ow_shuppetPal[16];
extern const u8 gfx_ow_banetteTiles[];
extern const color_t gfx_ow_banettePal[16];
extern const u8 gfx_ow_vipitisTiles[];
extern const color_t gfx_ow_vipitisPal[16];
extern const u8 gfx_ow_sengoTiles[];
extern const color_t gfx_ow_sengoPal[16];
extern const u8 gfx_ow_relicanthTiles[];
extern const color_t gfx_ow_relicanthPal[16];
extern const u8 gfx_ow_stolluniorTiles[];
extern const color_t gfx_ow_stolluniorPal[16];
extern const u8 gfx_ow_stollrakTiles[];
extern const color_t gfx_ow_stollrakPal[16];
extern const u8 gfx_ow_stollossTiles[];
extern const color_t gfx_ow_stollossPal[16];
extern const u8 gfx_ow_formeoTiles[];
extern const color_t gfx_ow_formeoPal[16];
extern const u8 gfx_ow_wonneiraTiles[];
extern const color_t gfx_ow_wonneiraPal[16];
extern const u8 gfx_ow_mobaiTiles[];
extern const color_t gfx_ow_mobaiPal[16];
extern const u8 gfx_ow_liliepTiles[];
extern const color_t gfx_ow_liliepPal[16];
extern const u8 gfx_ow_wielieTiles[];
extern const color_t gfx_ow_wieliePal[16];
extern const u8 gfx_ow_anorithTiles[];
extern const color_t gfx_ow_anorithPal[16];
extern const u8 gfx_ow_armaldoTiles[];
extern const color_t gfx_ow_armaldoPal[16];
extern const u8 gfx_ow_traslaTiles[];
extern const color_t gfx_ow_traslaPal[16];
extern const u8 gfx_ow_kirliaTiles[];
extern const color_t gfx_ow_kirliaPal[16];
extern const u8 gfx_ow_guardevoirTiles[];
extern const color_t gfx_ow_guardevoirPal[16];
extern const u8 gfx_ow_kindwurmTiles[];
extern const color_t gfx_ow_kindwurmPal[16];
extern const u8 gfx_ow_draschelTiles[];
extern const color_t gfx_ow_draschelPal[16];
extern const u8 gfx_ow_brutalandaTiles[];
extern const color_t gfx_ow_brutalandaPal[16];
extern const u8 gfx_ow_tanhelTiles[];
extern const color_t gfx_ow_tanhelPal[16];
extern const u8 gfx_ow_metangTiles[];
extern const color_t gfx_ow_metangPal[16];
extern const u8 gfx_ow_metagrossTiles[];
extern const color_t gfx_ow_metagrossPal[16];
extern const u8 gfx_ow_regirockTiles[];
extern const color_t gfx_ow_regirockPal[16];
extern const u8 gfx_ow_regiceTiles[];
extern const color_t gfx_ow_regicePal[16];
extern const u8 gfx_ow_registeelTiles[];
extern const color_t gfx_ow_registeelPal[16];
extern const u8 gfx_ow_kyogreTiles[];
extern const color_t gfx_ow_kyogrePal[16];
extern const u8 gfx_ow_groudonTiles[];
extern const color_t gfx_ow_groudonPal[16];
extern const u8 gfx_ow_rayquazaTiles[];
extern const color_t gfx_ow_rayquazaPal[16];
extern const u8 gfx_ow_latiasTiles[];
extern const color_t gfx_ow_latiasPal[16];
extern const u8 gfx_ow_latiosTiles[];
extern const color_t gfx_ow_latiosPal[16];
extern const u8 gfx_ow_jirachiTiles[];
extern const color_t gfx_ow_jirachiPal[16];
extern const u8 gfx_ow_deoxysTiles[];
extern const color_t gfx_ow_deoxysPal[16];
extern const u8 gfx_ow_palimpalimTiles[];
extern const color_t gfx_ow_palimpalimPal[16];
extern const u8 gfx_ow_icognito_bTiles[];
extern const color_t gfx_ow_icognito_bPal[16];
extern const u8 gfx_ow_icognito_cTiles[];
extern const color_t gfx_ow_icognito_cPal[16];
extern const u8 gfx_ow_icognito_dTiles[];
extern const color_t gfx_ow_icognito_dPal[16];
extern const u8 gfx_ow_icognito_eTiles[];
extern const color_t gfx_ow_icognito_ePal[16];
extern const u8 gfx_ow_icognito_fTiles[];
extern const color_t gfx_ow_icognito_fPal[16];
extern const u8 gfx_ow_icognito_gTiles[];
extern const color_t gfx_ow_icognito_gPal[16];
extern const u8 gfx_ow_icognito_hTiles[];
extern const color_t gfx_ow_icognito_hPal[16];
extern const u8 gfx_ow_icognito_iTiles[];
extern const color_t gfx_ow_icognito_iPal[16];
extern const u8 gfx_ow_icognito_jTiles[];
extern const color_t gfx_ow_icognito_jPal[16];
extern const u8 gfx_ow_icognito_kTiles[];
extern const color_t gfx_ow_icognito_kPal[16];
extern const u8 gfx_ow_icognito_lTiles[];
extern const color_t gfx_ow_icognito_lPal[16];
extern const u8 gfx_ow_icognito_mTiles[];
extern const color_t gfx_ow_icognito_mPal[16];
extern const u8 gfx_ow_icognito_nTiles[];
extern const color_t gfx_ow_icognito_nPal[16];
extern const u8 gfx_ow_icognito_oTiles[];
extern const color_t gfx_ow_icognito_oPal[16];
extern const u8 gfx_ow_icognito_pTiles[];
extern const color_t gfx_ow_icognito_pPal[16];
extern const u8 gfx_ow_icognito_qTiles[];
extern const color_t gfx_ow_icognito_qPal[16];
extern const u8 gfx_ow_icognito_rTiles[];
extern const color_t gfx_ow_icognito_rPal[16];
extern const u8 gfx_ow_icognito_sTiles[];
extern const color_t gfx_ow_icognito_sPal[16];
extern const u8 gfx_ow_icognito_tTiles[];
extern const color_t gfx_ow_icognito_tPal[16];
extern const u8 gfx_ow_icognito_uTiles[];
extern const color_t gfx_ow_icognito_uPal[16];
extern const u8 gfx_ow_icognito_vTiles[];
extern const color_t gfx_ow_icognito_vPal[16];
extern const u8 gfx_ow_icognito_wTiles[];
extern const color_t gfx_ow_icognito_wPal[16];
extern const u8 gfx_ow_icognito_xTiles[];
extern const color_t gfx_ow_icognito_xPal[16];
extern const u8 gfx_ow_icognito_yTiles[];
extern const color_t gfx_ow_icognito_yPal[16];
extern const u8 gfx_ow_icognito_zTiles[];
extern const color_t gfx_ow_icognito_zPal[16];
extern const u8 gfx_ow_icognito_exclamationTiles[];
extern const color_t gfx_ow_icognito_exclamationPal[16];
extern const u8 gfx_ow_icognito_questionTiles[];
extern const color_t gfx_ow_icognito_questionPal[16];
extern const u8 gfx_ow_molunkTiles[];
extern const color_t gfx_ow_molunkPal[16];
extern const u8 gfx_ow_amfiraTiles[];
extern const color_t gfx_ow_amfiraPal[16];
extern const u8 gfx_ow_pichuTiles[];
extern const color_t gfx_ow_pichuPal[16];
extern const u8 gfx_ow_rotomTiles[];
extern const color_t gfx_ow_rotomPal[16];
extern const u8 gfx_ow_rotom_heatTiles[];
extern const color_t gfx_ow_rotom_heatPal[16];
extern const u8 gfx_ow_rotom_frostTiles[];
extern const color_t gfx_ow_rotom_frostPal[16];
extern const u8 gfx_ow_rotom_mowTiles[];
extern const color_t gfx_ow_rotom_mowPal[16];
extern const u8 gfx_ow_rotom_washTiles[];
extern const color_t gfx_ow_rotom_washPal[16];
extern const u8 gfx_ow_rotom_fanTiles[];
extern const color_t gfx_ow_rotom_fanPal[16];
extern const u8 gfx_ow_crystal_onixTiles[];
extern const color_t gfx_ow_crystal_onixPal[16];


extern const unsigned short gfx_ow_deoxys_normal_0Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_1Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_2Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_3Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_4Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_5Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_6Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_7Tiles[];
extern const unsigned short gfx_ow_deoxys_normal_8Tiles[];
extern const unsigned short gfx_ow_registeel_0Tiles[];
extern const unsigned short gfx_ow_registeel_1Tiles[];
extern const unsigned short gfx_ow_registeel_2Tiles[];
extern const unsigned short gfx_ow_registeel_3Tiles[];
extern const unsigned short gfx_ow_registeel_4Tiles[];
extern const unsigned short gfx_ow_registeel_5Tiles[];
extern const unsigned short gfx_ow_registeel_6Tiles[];
extern const unsigned short gfx_ow_registeel_7Tiles[];
extern const unsigned short gfx_ow_registeel_8Tiles[];
extern const unsigned short gfx_ow_regice_0Tiles[];
extern const unsigned short gfx_ow_regice_1Tiles[];
extern const unsigned short gfx_ow_regice_2Tiles[];
extern const unsigned short gfx_ow_regice_3Tiles[];
extern const unsigned short gfx_ow_regice_4Tiles[];
extern const unsigned short gfx_ow_regice_5Tiles[];
extern const unsigned short gfx_ow_regice_6Tiles[];
extern const unsigned short gfx_ow_regice_7Tiles[];
extern const unsigned short gfx_ow_regice_8Tiles[];
extern const unsigned short gfx_ow_regirock_0Tiles[];
extern const unsigned short gfx_ow_regirock_1Tiles[];
extern const unsigned short gfx_ow_regirock_2Tiles[];
extern const unsigned short gfx_ow_regirock_3Tiles[];
extern const unsigned short gfx_ow_regirock_4Tiles[];
extern const unsigned short gfx_ow_regirock_5Tiles[];
extern const unsigned short gfx_ow_regirock_6Tiles[];
extern const unsigned short gfx_ow_regirock_7Tiles[];
extern const unsigned short gfx_ow_regirock_8Tiles[];
extern const unsigned short gfx_ow_voltilamm_0Tiles[];
extern const unsigned short gfx_ow_voltilamm_1Tiles[];
extern const unsigned short gfx_ow_voltilamm_2Tiles[];
extern const unsigned short gfx_ow_voltilamm_3Tiles[];
extern const unsigned short gfx_ow_voltilamm_4Tiles[];
extern const unsigned short gfx_ow_voltilamm_5Tiles[];
extern const unsigned short gfx_ow_voltilamm_6Tiles[];
extern const unsigned short gfx_ow_voltilamm_7Tiles[];
extern const unsigned short gfx_ow_voltilamm_8Tiles[];
extern const unsigned short gfx_ow_staralili_0Tiles[];
extern const unsigned short gfx_ow_staralili_1Tiles[];
extern const unsigned short gfx_ow_staralili_2Tiles[];
extern const unsigned short gfx_ow_staralili_3Tiles[];
extern const unsigned short gfx_ow_staralili_4Tiles[];
extern const unsigned short gfx_ow_staralili_5Tiles[];
extern const unsigned short gfx_ow_staralili_6Tiles[];
extern const unsigned short gfx_ow_staralili_7Tiles[];
extern const unsigned short gfx_ow_staralili_8Tiles[];
extern const unsigned short gfx_ow_larissa_0Tiles[];
extern const unsigned short gfx_ow_larissa_1Tiles[];
extern const unsigned short gfx_ow_larissa_2Tiles[];
extern const unsigned short gfx_ow_larissa_3Tiles[];
extern const unsigned short gfx_ow_larissa_4Tiles[];
extern const unsigned short gfx_ow_larissa_5Tiles[];
extern const unsigned short gfx_ow_larissa_6Tiles[];
extern const unsigned short gfx_ow_larissa_7Tiles[];
extern const unsigned short gfx_ow_larissa_8Tiles[];
extern const unsigned short gfx_ow_elise_0Tiles[];
extern const unsigned short gfx_ow_elise_1Tiles[];
extern const unsigned short gfx_ow_elise_2Tiles[];
extern const unsigned short gfx_ow_elise_3Tiles[];
extern const unsigned short gfx_ow_elise_4Tiles[];
extern const unsigned short gfx_ow_elise_5Tiles[];
extern const unsigned short gfx_ow_elise_6Tiles[];
extern const unsigned short gfx_ow_elise_7Tiles[];
extern const unsigned short gfx_ow_elise_8Tiles[];
extern const unsigned short gfx_ow_faun_young_0Tiles[];
extern const unsigned short gfx_ow_faun_young_1Tiles[];
extern const unsigned short gfx_ow_faun_young_2Tiles[];
extern const unsigned short gfx_ow_faun_young_3Tiles[];
extern const unsigned short gfx_ow_faun_young_4Tiles[];
extern const unsigned short gfx_ow_faun_young_5Tiles[];
extern const unsigned short gfx_ow_faun_young_6Tiles[];
extern const unsigned short gfx_ow_faun_young_7Tiles[];
extern const unsigned short gfx_ow_faun_young_8Tiles[];
extern const unsigned short gfx_ow_egg_0Tiles[];
extern const unsigned short gfx_ow_rin_0Tiles[];
extern const unsigned short gfx_ow_rin_1Tiles[];
extern const unsigned short gfx_ow_rin_2Tiles[];
extern const unsigned short gfx_ow_rin_3Tiles[];
extern const unsigned short gfx_ow_rin_4Tiles[];
extern const unsigned short gfx_ow_rin_5Tiles[];
extern const unsigned short gfx_ow_rin_6Tiles[];
extern const unsigned short gfx_ow_rin_7Tiles[];
extern const unsigned short gfx_ow_rin_8Tiles[];
extern const unsigned short gfx_ow_blaise_0Tiles[];
extern const unsigned short gfx_ow_blaise_1Tiles[];
extern const unsigned short gfx_ow_blaise_2Tiles[];
extern const unsigned short gfx_ow_blaise_3Tiles[];
extern const unsigned short gfx_ow_blaise_4Tiles[];
extern const unsigned short gfx_ow_blaise_5Tiles[];
extern const unsigned short gfx_ow_blaise_6Tiles[];
extern const unsigned short gfx_ow_blaise_7Tiles[];
extern const unsigned short gfx_ow_blaise_8Tiles[];
extern const unsigned short gfx_ow_mia_0Tiles[];
extern const unsigned short gfx_ow_mia_1Tiles[];
extern const unsigned short gfx_ow_mia_2Tiles[];
extern const unsigned short gfx_ow_mia_3Tiles[];
extern const unsigned short gfx_ow_mia_4Tiles[];
extern const unsigned short gfx_ow_mia_5Tiles[];
extern const unsigned short gfx_ow_mia_6Tiles[];
extern const unsigned short gfx_ow_mia_7Tiles[];
extern const unsigned short gfx_ow_mia_8Tiles[];
extern const u8 gfx_ow_lesterTiles[];
extern const unsigned short gfx_ow_faun_0Tiles[];
extern const unsigned short gfx_ow_faun_1Tiles[];
extern const unsigned short gfx_ow_faun_2Tiles[];
extern const unsigned short gfx_ow_faun_3Tiles[];
extern const unsigned short gfx_ow_faun_4Tiles[];
extern const unsigned short gfx_ow_faun_5Tiles[];
extern const unsigned short gfx_ow_faun_6Tiles[];
extern const unsigned short gfx_ow_faun_7Tiles[];
extern const unsigned short gfx_ow_faun_8Tiles[];
extern const unsigned short gfx_ow_revolutionary_0Tiles[];
extern const unsigned short gfx_ow_revolutionary_1Tiles[];
extern const unsigned short gfx_ow_revolutionary_2Tiles[];
extern const unsigned short gfx_ow_revolutionary_3Tiles[];
extern const unsigned short gfx_ow_revolutionary_4Tiles[];
extern const unsigned short gfx_ow_revolutionary_5Tiles[];
extern const unsigned short gfx_ow_revolutionary_6Tiles[];
extern const unsigned short gfx_ow_revolutionary_7Tiles[];
extern const unsigned short gfx_ow_revolutionary_8Tiles[];
extern const unsigned short gfx_ow_rival_0Tiles[];
extern const unsigned short gfx_ow_rival_1Tiles[];
extern const unsigned short gfx_ow_rival_2Tiles[];
extern const unsigned short gfx_ow_rival_3Tiles[];
extern const unsigned short gfx_ow_rival_4Tiles[];
extern const unsigned short gfx_ow_rival_5Tiles[];
extern const unsigned short gfx_ow_rival_6Tiles[];
extern const unsigned short gfx_ow_rival_7Tiles[];
extern const unsigned short gfx_ow_rival_8Tiles[];
extern const unsigned short gfx_ow_rival_beatenTiles[];
extern const unsigned short gfx_ow_tann_0Tiles[];
extern const unsigned short gfx_ow_tann_1Tiles[];
extern const unsigned short gfx_ow_tann_2Tiles[];
extern const unsigned short gfx_ow_tann_3Tiles[];
extern const unsigned short gfx_ow_tann_4Tiles[];
extern const unsigned short gfx_ow_tann_5Tiles[];
extern const unsigned short gfx_ow_tann_6Tiles[];
extern const unsigned short gfx_ow_tann_7Tiles[];
extern const unsigned short gfx_ow_tann_8Tiles[];
extern const unsigned short gfx_ow_felix_0Tiles[];
extern const unsigned short gfx_ow_felix_1Tiles[];
extern const unsigned short gfx_ow_felix_2Tiles[];
extern const unsigned short gfx_ow_felix_3Tiles[];
extern const unsigned short gfx_ow_felix_4Tiles[];
extern const unsigned short gfx_ow_felix_5Tiles[];
extern const unsigned short gfx_ow_felix_6Tiles[];
extern const unsigned short gfx_ow_felix_7Tiles[];
extern const unsigned short gfx_ow_felix_8Tiles[];
extern const unsigned short gfx_ow_albus_0Tiles[];
extern const unsigned short gfx_ow_albus_1Tiles[];
extern const unsigned short gfx_ow_albus_2Tiles[];
extern const unsigned short gfx_ow_albus_3Tiles[];
extern const unsigned short gfx_ow_albus_4Tiles[];
extern const unsigned short gfx_ow_albus_5Tiles[];
extern const unsigned short gfx_ow_albus_6Tiles[];
extern const unsigned short gfx_ow_albus_7Tiles[];
extern const unsigned short gfx_ow_albus_8Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_0Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_1Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_2Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_3Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_4Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_5Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_6Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_7Tiles[];
extern const unsigned short gfx_ow_violet_grunt_f_8Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_0Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_1Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_2Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_3Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_4Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_5Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_6Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_7Tiles[];
extern const unsigned short gfx_ow_violet_grunt_m_8Tiles[];
extern const unsigned short gfx_ow_primus_0Tiles[];
extern const unsigned short gfx_ow_primus_1Tiles[];
extern const unsigned short gfx_ow_primus_2Tiles[];
extern const unsigned short gfx_ow_primus_3Tiles[];
extern const unsigned short gfx_ow_primus_4Tiles[];
extern const unsigned short gfx_ow_primus_5Tiles[];
extern const unsigned short gfx_ow_primus_6Tiles[];
extern const unsigned short gfx_ow_primus_7Tiles[];
extern const unsigned short gfx_ow_primus_8Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_0Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_1Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_2Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_3Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_4Tiles[];
extern const unsigned short gfx_ow_hiro_call_bike_5Tiles[];
extern const unsigned short gfx_ow_hiro_call2_0Tiles[];
extern const unsigned short gfx_ow_hiro_call2_1Tiles[];
extern const unsigned short gfx_ow_hiro_call2_2Tiles[];
extern const unsigned short gfx_ow_hiro_call2_3Tiles[];
extern const unsigned short gfx_ow_hiro_call2_4Tiles[];
extern const unsigned short gfx_ow_hiro_call2_5Tiles[];
extern const unsigned short gfx_ow_hiro_call2_6Tiles[];
extern const unsigned short gfx_ow_hiro_call2_7Tiles[];
extern const unsigned short gfx_ow_hiro_call2_8Tiles[];
extern const unsigned short gfx_ow_hiro_rod_0Tiles[];
extern const unsigned short gfx_ow_hiro_rod_1Tiles[];
extern const unsigned short gfx_ow_hiro_rod_2Tiles[];
extern const unsigned short gfx_ow_hiro_rod_3Tiles[];
extern const unsigned short gfx_ow_hiro_rod_4Tiles[];
extern const unsigned short gfx_ow_hiro_rod_5Tiles[];
extern const unsigned short gfx_ow_hiro_rod_6Tiles[];
extern const unsigned short gfx_ow_hiro_rod_7Tiles[];
extern const unsigned short gfx_ow_hiro_rod_8Tiles[];
extern const unsigned short gfx_ow_hiro_rod_9Tiles[];
extern const unsigned short gfx_ow_hiro_rod_10Tiles[];
extern const unsigned short gfx_ow_hiro_rod_11Tiles[];
extern const unsigned short gfx_ow_hiro_call_0Tiles[];
extern const unsigned short gfx_ow_hiro_call_1Tiles[];
extern const unsigned short gfx_ow_hiro_call_2Tiles[];
extern const unsigned short gfx_ow_hiro_call_3Tiles[];
extern const unsigned short gfx_ow_hiro_call_4Tiles[];
extern const unsigned short gfx_ow_hiro_call_5Tiles[];
extern const unsigned short gfx_ow_hiro_call_6Tiles[];
extern const unsigned short gfx_ow_hiro_call_7Tiles[];
extern const unsigned short gfx_ow_hiro_call_8Tiles[];
extern const unsigned short gfx_ow_hiro_sit_0Tiles[];
extern const unsigned short gfx_ow_hiro_sit_1Tiles[];
extern const unsigned short gfx_ow_hiro_sit_2Tiles[];
extern const unsigned short gfx_ow_hiro_sit_3Tiles[];
extern const unsigned short gfx_ow_hiro_sit_4Tiles[];
extern const unsigned short gfx_ow_hiro_sit_5Tiles[];
extern const unsigned short gfx_ow_hiro_sit_6Tiles[];
extern const unsigned short gfx_ow_hiro_sit_7Tiles[];
extern const unsigned short gfx_ow_hiro_sit_8Tiles[];
extern const unsigned short gfx_ow_hiro_sit_9Tiles[];
extern const unsigned short gfx_ow_hiro_sit_10Tiles[];
extern const unsigned short gfx_ow_hiro_sit_11Tiles[];
extern const unsigned short gfx_ow_hiro_0Tiles[];
extern const unsigned short gfx_ow_hiro_1Tiles[];
extern const unsigned short gfx_ow_hiro_2Tiles[];
extern const unsigned short gfx_ow_hiro_3Tiles[];
extern const unsigned short gfx_ow_hiro_4Tiles[];
extern const unsigned short gfx_ow_hiro_5Tiles[];
extern const unsigned short gfx_ow_hiro_6Tiles[];
extern const unsigned short gfx_ow_hiro_7Tiles[];
extern const unsigned short gfx_ow_hiro_8Tiles[];
extern const unsigned short gfx_ow_hiro_9Tiles[];
extern const unsigned short gfx_ow_hiro_10Tiles[];
extern const unsigned short gfx_ow_hiro_11Tiles[];
extern const unsigned short gfx_ow_hiro_12Tiles[];
extern const unsigned short gfx_ow_hiro_13Tiles[];
extern const unsigned short gfx_ow_hiro_14Tiles[];
extern const unsigned short gfx_ow_hiro_15Tiles[];
extern const unsigned short gfx_ow_hiro_16Tiles[];
extern const unsigned short gfx_ow_hiro_17Tiles[];
extern const unsigned short gfx_ow_hiro_18Tiles[];
extern const unsigned short gfx_ow_hiro_19Tiles[];
extern const unsigned short gfx_ow_hiroine_0Tiles[];
extern const unsigned short gfx_ow_hiroine_1Tiles[];
extern const unsigned short gfx_ow_hiroine_2Tiles[];
extern const unsigned short gfx_ow_hiroine_3Tiles[];
extern const unsigned short gfx_ow_hiroine_4Tiles[];
extern const unsigned short gfx_ow_hiroine_5Tiles[];
extern const unsigned short gfx_ow_hiroine_6Tiles[];
extern const unsigned short gfx_ow_hiroine_7Tiles[];
extern const unsigned short gfx_ow_hiroine_8Tiles[];
extern const unsigned short gfx_ow_hiroine_9Tiles[];
extern const unsigned short gfx_ow_hiroine_10Tiles[];
extern const unsigned short gfx_ow_hiroine_11Tiles[];
extern const unsigned short gfx_ow_hiroine_12Tiles[];
extern const unsigned short gfx_ow_hiroine_13Tiles[];
extern const unsigned short gfx_ow_hiroine_14Tiles[];
extern const unsigned short gfx_ow_hiroine_15Tiles[];
extern const unsigned short gfx_ow_hiroine_16Tiles[];
extern const unsigned short gfx_ow_hiroine_17Tiles[];
extern const unsigned short gfx_ow_hiroine_18Tiles[];
extern const unsigned short gfx_ow_hiroine_19Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_0Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_1Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_2Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_3Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_4Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_5Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_6Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_7Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_8Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_9Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_10Tiles[];
extern const unsigned short gfx_ow_hiroine_rod_11Tiles[];
extern const unsigned short gfx_ow_hiroine_call_0Tiles[];
extern const unsigned short gfx_ow_hiroine_call_1Tiles[];
extern const unsigned short gfx_ow_hiroine_call_2Tiles[];
extern const unsigned short gfx_ow_hiroine_call_3Tiles[];
extern const unsigned short gfx_ow_hiroine_call_4Tiles[];
extern const unsigned short gfx_ow_hiroine_call_5Tiles[];
extern const unsigned short gfx_pskate_0Tiles[];
extern const unsigned short gfx_pskate_1Tiles[];
extern const unsigned short gfx_pskate_2Tiles[];
extern const unsigned short gfx_pskate_3Tiles[];
extern const unsigned short gfx_pskate_4Tiles[];
extern const unsigned short gfx_pskate_5Tiles[];
extern const unsigned short gfx_pskate_6Tiles[];
extern const unsigned short gfx_pskate_7Tiles[];
extern const unsigned short gfx_pskate_8Tiles[];
extern const unsigned short gfx_pskatef_0Tiles[];
extern const unsigned short gfx_pskatef_1Tiles[];
extern const unsigned short gfx_pskatef_2Tiles[];
extern const unsigned short gfx_pskatef_3Tiles[];
extern const unsigned short gfx_pskatef_4Tiles[];
extern const unsigned short gfx_pskatef_5Tiles[];
extern const unsigned short gfx_pskatef_6Tiles[];
extern const unsigned short gfx_pskatef_7Tiles[];
extern const unsigned short gfx_pskatef_8Tiles[];
extern const unsigned short gfx_ow_igva_0Tiles[];
extern const unsigned short gfx_ow_igva_0Tiles[];
extern const unsigned short gfx_ow_igva_1Tiles[];
extern const unsigned short gfx_ow_igva_1Tiles[];
extern const unsigned short gfx_ow_igva_2Tiles[];
extern const unsigned short gfx_ow_igva_2Tiles[];
extern const unsigned short gfx_ow_igva_3Tiles[];
extern const unsigned short gfx_ow_igva_3Tiles[];
extern const unsigned short gfx_ow_igva_4Tiles[];
extern const unsigned short gfx_ow_igva_4Tiles[];
extern const unsigned short gfx_ow_igva_5Tiles[];
extern const unsigned short gfx_ow_igva_5Tiles[];
extern const unsigned short gfx_ow_igva_6Tiles[];
extern const unsigned short gfx_ow_igva_6Tiles[];
extern const unsigned short gfx_ow_igva_7Tiles[];
extern const unsigned short gfx_ow_igva_7Tiles[];
extern const unsigned short gfx_ow_igva_8Tiles[];
extern const unsigned short gfx_ow_igva_8Tiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_bbshipTiles[];
extern const unsigned short gfx_ow_mpirate_0Tiles[];
extern const unsigned short gfx_ow_mpirate_0Tiles[];
extern const unsigned short gfx_ow_mpirate_1Tiles[];
extern const unsigned short gfx_ow_mpirate_1Tiles[];
extern const unsigned short gfx_ow_mpirate_2Tiles[];
extern const unsigned short gfx_ow_mpirate_2Tiles[];
extern const unsigned short gfx_ow_mpirate_3Tiles[];
extern const unsigned short gfx_ow_mpirate_3Tiles[];
extern const unsigned short gfx_ow_mpirate_4Tiles[];
extern const unsigned short gfx_ow_mpirate_4Tiles[];
extern const unsigned short gfx_ow_mpirate_5Tiles[];
extern const unsigned short gfx_ow_mpirate_5Tiles[];
extern const unsigned short gfx_ow_mpirate_6Tiles[];
extern const unsigned short gfx_ow_mpirate_6Tiles[];
extern const unsigned short gfx_ow_mpirate_7Tiles[];
extern const unsigned short gfx_ow_mpirate_7Tiles[];
extern const unsigned short gfx_ow_mpirate_8Tiles[];
extern const unsigned short gfx_ow_mpirate_8Tiles[];
extern const unsigned short gfx_ow_shape_animal_0Tiles[];
extern const unsigned short gfx_ow_shape_animal_0Tiles[];
extern const unsigned short gfx_ow_shape_animal_1Tiles[];
extern const unsigned short gfx_ow_shape_animal_1Tiles[];
extern const unsigned short gfx_ow_shape_animal_2Tiles[];
extern const unsigned short gfx_ow_shape_animal_2Tiles[];
extern const unsigned short gfx_ow_shape_animal_0Tiles[];
extern const unsigned short gfx_ow_shape_animal_0Tiles[];
extern const unsigned short gfx_ow_shape_animal_1Tiles[];
extern const unsigned short gfx_ow_shape_animal_1Tiles[];
extern const unsigned short gfx_ow_shape_animal_2Tiles[];
extern const unsigned short gfx_ow_shape_animal_2Tiles[];
extern const unsigned short gfx_ow_shape_human_0Tiles[];
extern const unsigned short gfx_ow_shape_human_0Tiles[];
extern const unsigned short gfx_ow_shape_human_1Tiles[];
extern const unsigned short gfx_ow_shape_human_1Tiles[];
extern const unsigned short gfx_ow_shape_human_2Tiles[];
extern const unsigned short gfx_ow_shape_human_2Tiles[];
extern const unsigned short gfx_ow_shape_human_0Tiles[];
extern const unsigned short gfx_ow_shape_human_0Tiles[];
extern const unsigned short gfx_ow_shape_human_1Tiles[];
extern const unsigned short gfx_ow_shape_human_1Tiles[];
extern const unsigned short gfx_ow_shape_human_2Tiles[];
extern const unsigned short gfx_ow_shape_human_2Tiles[];
extern const unsigned short gfx_ow_shape_tail_0Tiles[];
extern const unsigned short gfx_ow_shape_tail_0Tiles[];
extern const unsigned short gfx_ow_shape_tail_1Tiles[];
extern const unsigned short gfx_ow_shape_tail_1Tiles[];
extern const unsigned short gfx_ow_shape_tail_2Tiles[];
extern const unsigned short gfx_ow_shape_tail_2Tiles[];
extern const unsigned short gfx_ow_shape_tail_0Tiles[];
extern const unsigned short gfx_ow_shape_tail_0Tiles[];
extern const unsigned short gfx_ow_shape_tail_1Tiles[];
extern const unsigned short gfx_ow_shape_tail_1Tiles[];
extern const unsigned short gfx_ow_shape_tail_2Tiles[];
extern const unsigned short gfx_ow_shape_tail_2Tiles[];
extern const unsigned short gfx_ow_shape_snake_0Tiles[];
extern const unsigned short gfx_ow_shape_snake_0Tiles[];
extern const unsigned short gfx_ow_shape_snake_1Tiles[];
extern const unsigned short gfx_ow_shape_snake_1Tiles[];
extern const unsigned short gfx_ow_shape_snake_2Tiles[];
extern const unsigned short gfx_ow_shape_snake_2Tiles[];
extern const unsigned short gfx_ow_shape_snake_0Tiles[];
extern const unsigned short gfx_ow_shape_snake_0Tiles[];
extern const unsigned short gfx_ow_shape_snake_1Tiles[];
extern const unsigned short gfx_ow_shape_snake_1Tiles[];
extern const unsigned short gfx_ow_shape_snake_2Tiles[];
extern const unsigned short gfx_ow_shape_snake_2Tiles[];
extern const unsigned short gfx_ow_shape_winsect_0Tiles[];
extern const unsigned short gfx_ow_shape_winsect_0Tiles[];
extern const unsigned short gfx_ow_shape_winsect_1Tiles[];
extern const unsigned short gfx_ow_shape_winsect_1Tiles[];
extern const unsigned short gfx_ow_shape_winsect_2Tiles[];
extern const unsigned short gfx_ow_shape_winsect_2Tiles[];
extern const unsigned short gfx_ow_shape_winsect_0Tiles[];
extern const unsigned short gfx_ow_shape_winsect_0Tiles[];
extern const unsigned short gfx_ow_shape_winsect_1Tiles[];
extern const unsigned short gfx_ow_shape_winsect_1Tiles[];
extern const unsigned short gfx_ow_shape_winsect_2Tiles[];
extern const unsigned short gfx_ow_shape_winsect_2Tiles[];
extern const unsigned short gfx_ow_shape_winged_0Tiles[];
extern const unsigned short gfx_ow_shape_winged_0Tiles[];
extern const unsigned short gfx_ow_shape_winged_1Tiles[];
extern const unsigned short gfx_ow_shape_winged_1Tiles[];
extern const unsigned short gfx_ow_shape_winged_2Tiles[];
extern const unsigned short gfx_ow_shape_winged_2Tiles[];
extern const unsigned short gfx_ow_shape_winged_0Tiles[];
extern const unsigned short gfx_ow_shape_winged_0Tiles[];
extern const unsigned short gfx_ow_shape_winged_1Tiles[];
extern const unsigned short gfx_ow_shape_winged_1Tiles[];
extern const unsigned short gfx_ow_shape_winged_2Tiles[];
extern const unsigned short gfx_ow_shape_winged_2Tiles[];
extern const unsigned short gfx_ow_shape_insect_0Tiles[];
extern const unsigned short gfx_ow_shape_insect_0Tiles[];
extern const unsigned short gfx_ow_shape_insect_1Tiles[];
extern const unsigned short gfx_ow_shape_insect_1Tiles[];
extern const unsigned short gfx_ow_shape_insect_2Tiles[];
extern const unsigned short gfx_ow_shape_insect_2Tiles[];
extern const unsigned short gfx_ow_shape_insect_0Tiles[];
extern const unsigned short gfx_ow_shape_insect_0Tiles[];
extern const unsigned short gfx_ow_shape_insect_1Tiles[];
extern const unsigned short gfx_ow_shape_insect_1Tiles[];
extern const unsigned short gfx_ow_shape_insect_2Tiles[];
extern const unsigned short gfx_ow_shape_insect_2Tiles[];
extern const unsigned short gfx_ow_shape_head_0Tiles[];
extern const unsigned short gfx_ow_shape_head_0Tiles[];
extern const unsigned short gfx_ow_shape_head_1Tiles[];
extern const unsigned short gfx_ow_shape_head_1Tiles[];
extern const unsigned short gfx_ow_shape_head_2Tiles[];
extern const unsigned short gfx_ow_shape_head_2Tiles[];
extern const unsigned short gfx_ow_shape_head_0Tiles[];
extern const unsigned short gfx_ow_shape_head_0Tiles[];
extern const unsigned short gfx_ow_shape_head_1Tiles[];
extern const unsigned short gfx_ow_shape_head_1Tiles[];
extern const unsigned short gfx_ow_shape_head_2Tiles[];
extern const unsigned short gfx_ow_shape_head_2Tiles[];
extern const unsigned short gfx_ow_shape_arms_0Tiles[];
extern const unsigned short gfx_ow_shape_arms_0Tiles[];
extern const unsigned short gfx_ow_shape_arms_1Tiles[];
extern const unsigned short gfx_ow_shape_arms_1Tiles[];
extern const unsigned short gfx_ow_shape_arms_2Tiles[];
extern const unsigned short gfx_ow_shape_arms_2Tiles[];
extern const unsigned short gfx_ow_shape_arms_0Tiles[];
extern const unsigned short gfx_ow_shape_arms_0Tiles[];
extern const unsigned short gfx_ow_shape_arms_1Tiles[];
extern const unsigned short gfx_ow_shape_arms_1Tiles[];
extern const unsigned short gfx_ow_shape_arms_2Tiles[];
extern const unsigned short gfx_ow_shape_arms_2Tiles[];
extern const unsigned short gfx_ow_shape_legs_0Tiles[];
extern const unsigned short gfx_ow_shape_legs_0Tiles[];
extern const unsigned short gfx_ow_shape_legs_1Tiles[];
extern const unsigned short gfx_ow_shape_legs_1Tiles[];
extern const unsigned short gfx_ow_shape_legs_2Tiles[];
extern const unsigned short gfx_ow_shape_legs_2Tiles[];
extern const unsigned short gfx_ow_shape_legs_0Tiles[];
extern const unsigned short gfx_ow_shape_legs_0Tiles[];
extern const unsigned short gfx_ow_shape_legs_1Tiles[];
extern const unsigned short gfx_ow_shape_legs_1Tiles[];
extern const unsigned short gfx_ow_shape_legs_2Tiles[];
extern const unsigned short gfx_ow_shape_legs_2Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_0Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_0Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_1Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_1Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_2Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_2Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_0Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_0Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_1Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_1Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_2Tiles[];
extern const unsigned short gfx_ow_shape_tentacles_2Tiles[];
extern const unsigned short gfx_ow_shape_fish_0Tiles[];
extern const unsigned short gfx_ow_shape_fish_0Tiles[];
extern const unsigned short gfx_ow_shape_fish_1Tiles[];
extern const unsigned short gfx_ow_shape_fish_1Tiles[];
extern const unsigned short gfx_ow_shape_fish_2Tiles[];
extern const unsigned short gfx_ow_shape_fish_2Tiles[];
extern const unsigned short gfx_ow_shape_fish_0Tiles[];
extern const unsigned short gfx_ow_shape_fish_0Tiles[];
extern const unsigned short gfx_ow_shape_fish_1Tiles[];
extern const unsigned short gfx_ow_shape_fish_1Tiles[];
extern const unsigned short gfx_ow_shape_fish_2Tiles[];
extern const unsigned short gfx_ow_shape_fish_2Tiles[];
extern const unsigned short gfx_ow_shape_ball_0Tiles[];
extern const unsigned short gfx_ow_shape_ball_0Tiles[];
extern const unsigned short gfx_ow_shape_ball_1Tiles[];
extern const unsigned short gfx_ow_shape_ball_1Tiles[];
extern const unsigned short gfx_ow_shape_ball_2Tiles[];
extern const unsigned short gfx_ow_shape_ball_2Tiles[];
extern const unsigned short gfx_ow_shape_ball_0Tiles[];
extern const unsigned short gfx_ow_shape_ball_0Tiles[];
extern const unsigned short gfx_ow_shape_ball_1Tiles[];
extern const unsigned short gfx_ow_shape_ball_1Tiles[];
extern const unsigned short gfx_ow_shape_ball_2Tiles[];
extern const unsigned short gfx_ow_shape_ball_2Tiles[];
extern const unsigned short gfx_ow_shape_multi_0Tiles[];
extern const unsigned short gfx_ow_shape_multi_0Tiles[];
extern const unsigned short gfx_ow_shape_multi_1Tiles[];
extern const unsigned short gfx_ow_shape_multi_1Tiles[];
extern const unsigned short gfx_ow_shape_multi_2Tiles[];
extern const unsigned short gfx_ow_shape_multi_2Tiles[];
extern const unsigned short gfx_ow_shape_multi_0Tiles[];
extern const unsigned short gfx_ow_shape_multi_0Tiles[];
extern const unsigned short gfx_ow_shape_multi_1Tiles[];
extern const unsigned short gfx_ow_shape_multi_1Tiles[];
extern const unsigned short gfx_ow_shape_multi_2Tiles[];
extern const unsigned short gfx_ow_shape_multi_2Tiles[];
extern const unsigned short gfx_ow_mistral_0Tiles[];
extern const unsigned short gfx_ow_mistral_0Tiles[];
extern const unsigned short gfx_ow_mistral_1Tiles[];
extern const unsigned short gfx_ow_mistral_1Tiles[];
extern const unsigned short gfx_ow_mistral_2Tiles[];
extern const unsigned short gfx_ow_mistral_2Tiles[];
extern const unsigned short gfx_ow_mistral_3Tiles[];
extern const unsigned short gfx_ow_mistral_3Tiles[];
extern const unsigned short gfx_ow_mistral_4Tiles[];
extern const unsigned short gfx_ow_mistral_4Tiles[];
extern const unsigned short gfx_ow_mistral_5Tiles[];
extern const unsigned short gfx_ow_mistral_5Tiles[];
extern const unsigned short gfx_ow_mistral_6Tiles[];
extern const unsigned short gfx_ow_mistral_6Tiles[];
extern const unsigned short gfx_ow_mistral_7Tiles[];
extern const unsigned short gfx_ow_mistral_7Tiles[];
extern const unsigned short gfx_ow_mistral_8Tiles[];
extern const unsigned short gfx_ow_mistral_8Tiles[];
extern const unsigned short gfx_ow_manus_0Tiles[];
extern const unsigned short gfx_ow_manus_0Tiles[];
extern const unsigned short gfx_ow_manus_1Tiles[];
extern const unsigned short gfx_ow_manus_1Tiles[];
extern const unsigned short gfx_ow_manus_2Tiles[];
extern const unsigned short gfx_ow_manus_2Tiles[];
extern const unsigned short gfx_ow_manus_3Tiles[];
extern const unsigned short gfx_ow_manus_3Tiles[];
extern const unsigned short gfx_ow_manus_4Tiles[];
extern const unsigned short gfx_ow_manus_4Tiles[];
extern const unsigned short gfx_ow_manus_5Tiles[];
extern const unsigned short gfx_ow_manus_5Tiles[];
extern const unsigned short gfx_ow_manus_6Tiles[];
extern const unsigned short gfx_ow_manus_6Tiles[];
extern const unsigned short gfx_ow_manus_7Tiles[];
extern const unsigned short gfx_ow_manus_7Tiles[];
extern const unsigned short gfx_ow_manus_8Tiles[];
extern const unsigned short gfx_ow_manus_8Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_1Tiles[];
extern const unsigned short gfx_ow_altaria_1Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_1Tiles[];
extern const unsigned short gfx_ow_altaria_0Tiles[];
extern const unsigned short gfx_ow_altaria_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_fem_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_0Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_1Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_2Tiles[];
extern const unsigned short gfx_ow_cloudrider_male_2Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_1Tiles[];
extern const unsigned short gfx_ow_moltres_1Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_1Tiles[];
extern const unsigned short gfx_ow_moltres_0Tiles[];
extern const unsigned short gfx_ow_moltres_1Tiles[];
extern const unsigned short gfx_ow_diserakt_0Tiles[];
extern const unsigned short gfx_ow_diserakt_0Tiles[];
extern const unsigned short gfx_ow_diserakt_2Tiles[];
extern const unsigned short gfx_ow_diserakt_2Tiles[];
extern const unsigned short gfx_ow_diserakt_1Tiles[];
extern const unsigned short gfx_ow_diserakt_1Tiles[];
extern const unsigned short gfx_ow_diserakt_0Tiles[];
extern const unsigned short gfx_ow_diserakt_0Tiles[];
extern const unsigned short gfx_ow_diserakt_2Tiles[];
extern const unsigned short gfx_ow_diserakt_2Tiles[];
extern const unsigned short gfx_ow_diserakt_1Tiles[];
extern const unsigned short gfx_ow_diserakt_1Tiles[];
extern const unsigned short gfx_ow_groudon_0Tiles[];
extern const unsigned short gfx_ow_groudon_0Tiles[];
extern const unsigned short gfx_ow_groudon_0Tiles[];
extern const unsigned short gfx_ow_groudon_0Tiles[];
extern const unsigned short gfx_ow_groudon_1Tiles[];
extern const unsigned short gfx_ow_groudon_1Tiles[];
extern const unsigned short gfx_ow_groudon_2Tiles[];
extern const unsigned short gfx_ow_groudon_2Tiles[];
extern const unsigned short gfx_ow_groudon_1Tiles[];
extern const unsigned short gfx_ow_groudon_2Tiles[];
extern const unsigned short gfx_ow_groudon_1Tiles[];
extern const unsigned short gfx_ow_groudon_2Tiles[];
extern const unsigned short gfx_ow_rubys_0Tiles[];
extern const unsigned short gfx_ow_rubys_0Tiles[];
extern const unsigned short gfx_ow_rubys_1Tiles[];
extern const unsigned short gfx_ow_rubys_1Tiles[];
extern const unsigned short gfx_ow_rubys_2Tiles[];
extern const unsigned short gfx_ow_rubys_2Tiles[];
extern const unsigned short gfx_ow_rubys_3Tiles[];
extern const unsigned short gfx_ow_rubys_3Tiles[];
extern const unsigned short gfx_ow_rubys_4Tiles[];
extern const unsigned short gfx_ow_rubys_4Tiles[];
extern const unsigned short gfx_ow_rubys_5Tiles[];
extern const unsigned short gfx_ow_rubys_5Tiles[];
extern const unsigned short gfx_ow_rubys_6Tiles[];
extern const unsigned short gfx_ow_rubys_6Tiles[];
extern const unsigned short gfx_ow_rubys_7Tiles[];
extern const unsigned short gfx_ow_rubys_7Tiles[];
extern const unsigned short gfx_ow_rubys_8Tiles[];
extern const unsigned short gfx_ow_rubys_8Tiles[];
extern const unsigned short gfx_ow_hiroine_sit_0Tiles[];
extern const unsigned short gfx_ow_hiroine_sit_1Tiles[];
extern const unsigned short gfx_ow_hiroine_sit_2Tiles[];
extern const unsigned short gfx_ow_botogel_0Tiles[];
extern const unsigned short gfx_ow_botogel_1Tiles[];
extern const unsigned short gfx_ow_botogel_2Tiles[];
extern const unsigned short gfx_ow_mother_0Tiles[];
extern const unsigned short gfx_ow_mother_1Tiles[];
extern const unsigned short gfx_ow_mother_2Tiles[];
extern const unsigned short gfx_ow_mother_3Tiles[];
extern const unsigned short gfx_ow_mother_4Tiles[];
extern const unsigned short gfx_ow_mother_5Tiles[];
extern const unsigned short gfx_ow_mother_6Tiles[];
extern const unsigned short gfx_ow_mother_7Tiles[];
extern const unsigned short gfx_ow_mother_8Tiles[];
extern const unsigned short gfx_ow_drampaTiles[];
extern const unsigned short gfx_ow_blackbeardTiles[];
extern const unsigned short gfx_ow_teahTiles[];
extern const unsigned short gfx_ow_narcissaTiles[];
extern const unsigned short gfx_ow_harrenfeld_0Tiles[];
extern const unsigned short gfx_ow_harrenfeld_1Tiles[];
extern const unsigned short gfx_ow_harrenfeld_2Tiles[];
extern const unsigned short gfx_ow_harrenfeld_3Tiles[];
extern const unsigned short gfx_ow_harrenfeld_4Tiles[];
extern const unsigned short gfx_ow_harrenfeld_5Tiles[];
extern const unsigned short gfx_ow_harrenfeld_6Tiles[];
extern const unsigned short gfx_ow_harrenfeld_7Tiles[];
extern const unsigned short gfx_ow_harrenfeld_8Tiles[];
extern const unsigned short gfx_ow_butler_0Tiles[];
extern const unsigned short gfx_ow_butler_1Tiles[];
extern const unsigned short gfx_ow_butler_2Tiles[];
extern const unsigned short gfx_ow_butler_3Tiles[];
extern const unsigned short gfx_ow_butler_4Tiles[];
extern const unsigned short gfx_ow_butler_5Tiles[];
extern const unsigned short gfx_ow_butler_6Tiles[];
extern const unsigned short gfx_ow_butler_7Tiles[];
extern const unsigned short gfx_ow_butler_8Tiles[];
extern const unsigned short gfx_ow_lucius_0Tiles[];
extern const unsigned short gfx_ow_lucius_1Tiles[];
extern const unsigned short gfx_ow_lucius_2Tiles[];
extern const unsigned short gfx_ow_lucius_3Tiles[];
extern const unsigned short gfx_ow_lucius_4Tiles[];
extern const unsigned short gfx_ow_lucius_5Tiles[];
extern const unsigned short gfx_ow_lucius_6Tiles[];
extern const unsigned short gfx_ow_lucius_7Tiles[];
extern const unsigned short gfx_ow_lucius_8Tiles[];
extern const u8 gfx_ow_luckyTiles[];
extern const u8 gfx_ow_chefTiles[];
extern const u8 gfx_ow_mega_stoneTiles[];
extern const u8 gfx_ow_lester_youngTiles[];
extern const u8 gfx_ow_romanTiles[];
extern const u8 gfx_ow_rageTiles[];
extern const u8 gfx_ow_rosalieTiles[];
extern const u8 gfx_ow_old_amberTiles[];
extern const u8 gfx_ow_icarusTiles[];
extern const u8 gfx_ow_appleTiles[];
extern const u8 gfx_ow_recipeTiles[];
extern const u8 gfx_ow_recipe2Tiles[];
extern const u8 gfx_ow_faunTiles[];
extern const u8 gfx_ow_blue_gearTiles[GRAPHIC_SIZE_4BPP(16, 16)];
extern const u8 gfx_ow_red_gearTiles[GRAPHIC_SIZE_4BPP(16, 16)];
extern const u8 gfx_ow_green_gearTiles[GRAPHIC_SIZE_4BPP(16, 16)];
extern const u8 gfx_ow_yellow_gearTiles[GRAPHIC_SIZE_4BPP(16, 16)];
extern const u8 gfx_ow_treasure_mapTiles[GRAPHIC_SIZE_4BPP(16, 16)];
extern const u8 gfx_ow_painterTiles[];

extern const u8 gfx_tutor_crystal_bugTiles[];
extern const color_t gfx_tutor_crystal_bugPal[16];
extern const color_t gfx_tutor_crystal_darkPal[16];
extern const color_t gfx_tutor_crystal_dragonPal[16];
extern const color_t gfx_tutor_crystal_electricPal[16];
extern const color_t gfx_tutor_crystal_fairyPal[16];
extern const color_t gfx_tutor_crystal_fightingPal[16];
extern const color_t gfx_tutor_crystal_firePal[16];
extern const color_t gfx_tutor_crystal_flyingPal[16];
extern const color_t gfx_tutor_crystal_ghostPal[16];
extern const color_t gfx_tutor_crystal_grassPal[16];
extern const color_t gfx_tutor_crystal_groundPal[16];
extern const color_t gfx_tutor_crystal_icePal[16];
extern const color_t gfx_tutor_crystal_normalPal[16];
extern const color_t gfx_tutor_crystal_poisonPal[16];
extern const color_t gfx_tutor_crystal_psychicPal[16];
extern const color_t gfx_tutor_crystal_rockPal[16];
extern const color_t gfx_tutor_crystal_steelPal[16];
extern const color_t gfx_tutor_crystal_waterPal[16];

extern const u8 gfx_ow_mushroomTiles[];
extern const color_t gfx_ow_mushroomPal[];
extern const u8 gfx_ow_shellTiles[];
extern const u8 gfx_ow_special_shellTiles[];
extern const color_t gfx_ow_shellPal[];
extern const color_t gfx_ow_special_shellPal[];
extern const u8 gfx_ow_boulder_gym_puzzleTiles[];
extern const u8 gfx_ow_boulder_hayTiles[];
extern const color_t gfx_ow_boulder_gym_puzzlePal[];
extern const color_t gfx_ow_mega_stonePal[];
extern const color_t gfx_ow_romanPal[];
extern const color_t gfx_ow_boulder_hayPal[];
extern const color_t gfx_ow_ragePal[];
extern const color_t gfx_ow_rosaliePal[];
extern const color_t gfx_ow_icarusPal[];
extern const color_t gfx_ow_painterPal[16];
extern const u8 gfx_ow_trashTiles[];
extern const color_t gfx_ow_trashPal[];
extern const u8 gfx_overworld_secret_power_vinesTiles[];
extern const color_t gfx_overworld_secret_power_vinesPal[];
extern const u8 gfx_overworld_secret_power_caveTiles[];
extern const color_t gfx_overworld_secret_power_cavePal[];
extern const color_t gfx_ow_applePal[16];
extern const color_t gfx_ow_recipePal[16];
extern const color_t gfx_ow_recipe2Pal[16];
extern const u8 gfx_ow_upstreamTiles[];
extern const color_t gfx_ow_upstreamPal[16];
extern const color_t gfx_ow_faunPal[];
extern const color_t gfx_ow_blue_gearPal[16];
extern const color_t gfx_ow_green_gearPal[16];
extern const color_t gfx_ow_red_gearPal[16];
extern const color_t gfx_ow_yellow_gearPal[16];
extern const color_t gfx_ow_treasure_mapPal[16];

extern const overworld_sprite overworld_sprite_strength_boulder;

#define OVERWORLD_PLAYER_PICTURE_CONTEXT_WALKING 0
#define OVERWORLD_PLAYER_PICTURE_CONTEXT_BIKING 1

/**
 * Gets the picture of the player in a given context.
 * @param context the context the player is in
 * @return the picture of the player
 **/
u8 player_get_overworld_picture(u8 context);


/**
 * Finds the overworld_sprite structure of a picture idx.
 * @param picture the picture of the overworld
 * @return sprite the overworld_sprite structure
 **/
const overworld_sprite *overworld_get(u16 picture);

/**
 * Translates map coordinates to coordinates for an oam.
 * @param map_x the x on the map in blocks
 * @param map_y the y on the map in blocks
 * @param oam_x resulting oam x coordinate
 * @param oam_y resulting oam y coordinate
 **/
void map_position_to_oam_position(s16 map_x, s16 map_y, s16 *oam_x, s16 *oam_y);

/**
 * Callback that handles (probably) applymovement stuff and movements for an npc while a script is not active
 * @param self self-reference
 **/
void npc_oam_callback_script_not_active (oam_object *self);

#endif /* INCLUDE_C_OVERWORLD_SPRITE_H_ */
