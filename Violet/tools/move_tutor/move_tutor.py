...
def get_move_tutors(project: pymap.project.Project) -> Dict:
    """ Gets move tutors in the project as a dict."""
    move_tutors = dict()
    ...
    for bank in project.headers:
        for map_idx in project.headers[bank]:
            ...
            for person_idx, person in enumerate(header['events']['persons']):
                try:
                    if person['script_std_and_in_connection']['script_std'] == 'PERSON_MOVE_TUTOR':
                        move_tutor_idx = person['value']['move_tutor']
                        if move_tutor_idx in move_tutors:
                            raise RuntimeError(f'Move tutor {move_tutor_idx} is used multiple times: {move_tutors[move_tutor_idx]}')
                    ...
                    print(f'Error in parsing move tutor {person_idx} on map {bank},{map_idx}')
                    raise e
    return move_tutors


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Creates metadata for move tutors')
    parser.add_argument('symbol', help='The symbol of the move tutors')
    ...
    move_tutors = get_move_tutors(project)

    
    data = []
    for i in range(len(project.constants['move_tutors'])):
        move_tutor_idx = project.constants['move_tutors'].inverse()[i]
        assert len(move_tutor_idx) == 1
        move_tutor_idx = move_tutor_idx[0]
        data.append(f'@ move tutor {i}, {move_tutor_idx}')
        if move_tutor_idx in move_tutors:
