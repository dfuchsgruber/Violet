...
entry_type = agb.types.Structure(
    [("level_min", "u8", 0), ("level_max", "u8", 0), ("species", "species", 0)]
)

entries_grass_type = agb.types.FixedSizeArrayType(
    "wild_pokemon.entry", (lambda project, context: 12)
)

entries_water_type = agb.types.FixedSizeArrayType(
    "wild_pokemon.entry", (lambda project, context: 5)
)
...
entries_odd_rod_type = agb.types.FixedSizeArrayType(
    "wild_pokemon.entry", (lambda project, context: 2)
)

entries_good_rod_type = agb.types.FixedSizeArrayType(
    "wild_pokemon.entry", (lambda project, context: 3)
)
...
wild_pokemon_list_with_pdf_entry_type = agb.types.Structure(
    [("species", "species", 0), ("level", "u8", 0), ("probability", "u8", 0)]
)

wild_pokemon_list_with_pdf_type = agb.types.UnboundedArrayType(
    "wild_pokemon.list_with_pdf_entry", {"species": "POKEMON_POKEMON_0", "level": 0, "probability": 0}
)
...
models_to_export = {
    ...
    "wild_pokemon.list_with_pdf_entry": wild_pokemon_list_with_pdf_entry_type,
    "wild_pokemon.list_with_pdf": wild_pokemon_list_with_pdf_type,
}
